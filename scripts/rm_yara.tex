...
If all correct mapping locations occur at optimal edit distance $o$, then $\theta_{0} = 1$ and $\theta_{e-o} = 0$ for all $e > o$.
...
and since
\begin{eqnarray}
\Pr[C' \not{\in} \Stratum'_{e'}] = 1 - \frac{1}{Z'} \cdot z_{e'} \theta_{e'},
\end{eqnarray}
it follows that
\begin{eqnarray}
\label{eq:yara:mqual:pe:unpaired}
\Pr[C = (i,j,e) | \Stratum^P_e = \emptyset] = \frac{1}{Z} \cdot \theta_e - \frac{1}{ZZ'} \cdot z_{e'} \theta_e \theta_{e'}.
\end{eqnarray}
...
\Pr[C \in \Stratum^P_e] = \frac{p' z^p_e \theta_e}{p' \cdot Z^P + (1-p') \cdot Z^I}
\end{eqnarray}
and the probability that any of these locations is correct as
\begin{eqnarray}
\label{eq:yara:mqual:pe:paired}
\Pr[C = (i,j,e) \in \Stratum^P_e] = \frac{p' \theta_e}{p' \cdot Z^P + (1-p') \cdot Z^I}.
\end{eqnarray}
