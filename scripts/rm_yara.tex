...
Yara works on Illumina or Ion Torrent reads, supports paired-end and mate-pair protocols, computes accurate mapping qualities, offers parallelization via multi-threading, has a low memory footprint thanks to the FM-index, and does not require ad-hoc parameterization.
...
Under any fixed filtration scheme, the number of verifications per read is not uniform: within a typical mapping, most reads produce very few verifications and are easily mappable, while few other reads are problematic and often not even confidently mappable to one single location.
...
Full-sensitive subsets of $\mathbb{k}$ are: $(0,0,-)$, $(0,-,0)$, $(-,0,0)$, $(1,-,-)$, $(-,1,-)$, $(-,-,1)$.
...
Thanks to this method, Yara in best-mapping is an order of magnitude faster than in conventional all-mapping.
...
Under these terms, no indexing space-time trade-off holds: the FM-index always provides the most convenient suffix trie implementation.

Yara does not use multiple search algorithms of section~\ref{sec:index:algo:multimismatch} to search seeds on the FM-index.
...
Using the FM-index, it is always faster to search exact queries in a na\"ive way and approximate queries after sorting them in lexicographical order (see section~\ref{sec:index:algo}).
