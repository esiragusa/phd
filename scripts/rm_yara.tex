...
Given a filtration scheme $\Scheme=(t_{\At{1}},\dots,t_{\AtEnd{s}})$ full-sensitive within distance $k$, any subset consisting of $s^* \leq s$ seeds with thresholds $\mathbf{t^*}=(t^{*}_{\At{1}},\dots,t^{*}_{\AtEnd{s^*}})$ is full-sensitive within distance $e^*+s$ if it satisfies
\begin{eqnarray}
...
Given the filtration scheme $\Scheme=(1,1,1)$, any subset full-sensitive up to distance $e^*+s = 1$ finds all relevant mapping locations.
...
Finally, the estimation of mapping qualities in Yara [s=0] can use matches from sub-optimal strata, which are found but not reported; this could render Yara [s=0] almost as good as Yara [s=1].
