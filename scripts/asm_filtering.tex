...
Both classes consist of various filtering methods.

I consider the following seed filtering methods:
\begin{inparaenum}[(i)]
\item exact seeds \citep{Baeza1992},
\item approximate seeds \citep{Myers1994,Navarro2000},
\item suffix filters \citep{Kaerkkaeinen2007}.
\end{inparaenum}
...
\begin{inparaenum}[(i)]
\item contiguous $q$-grams \citep{Jokinen1991},
\item gapped $q$-grams \citep{Burkhardt2001},
\item multiple gapped $q$-grams \citep{Kucherov2005}.
\end{inparaenum}
...
Multiple gapped $q$-grams apply multiple patterns of don't care positions in conjunction to further increase specificity.
...
%The simple analysis of section~\ref{sec:filtering:exact} shows that filtration specificity is strongly correlated to seed length.
%Therefore the crux of designing stronger filters lies into increasing the seed length while guaranteeing full-sensitivity.
...
Let $x,y$ be two strings \st $d_E(x,y) = k$.
If I partition \wlogs $y$ into $s$ non-overlapping seeds \st $1 \leq s \leq k+1$, then at least one seed occurs as a factor of $x$ within distance $\lfloor k/s \rfloor$.
\end{lemma}
