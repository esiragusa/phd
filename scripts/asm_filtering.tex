...
\begin{enumerate} %[(i)]
\item \label{enum:qgram-non-detection} Does filtration scheme $(Q,1)$ solve instance $(m,k)$? 
%\item If the answer to question~\ref{enum:qgram-non-detection} is yes:
%\begin{enumerate}
\item \label{enum:qgram-threshold} Which is the maximum threshold $t^*$ \st $(Q,t^*)$ solves $(m,k)$?
\item \label{enum:qgram-distance} Which is the minimum distance $k^*$ for which $(m,k^*)$ is not solved by $(Q,t)$?
%\end{enumerate}
...
\item \label{enum:qgram-fp} Which is the expected specificity of $(Q,t)$?
