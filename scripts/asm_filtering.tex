...
\cite{Califano1993} first introduced gapped $q$-grams in sequence analysis.
Since then, a surprisingly high number of research papers have been published on this topic (see \cite{Brown2008} for a survey).
...
\emph{Gapped $q$-grams} generalize contiguous $q$-grams by introducing patterns of \emph{don't care positions} that skip characters at fixed positions.
Figure \ref{fig:qgrams-ext} shows that one single mismatch (an $R$ in the Hamming distance transcript) affects a cluster of $q$ consecutive $q$-gram occurrences.
Conversely, as illustrated in figure \ref{fig:qgrams-gapped}, gapped $q$-gram occurrences are not affected by mismatches over don't care positions.
This fact relaxes the minimum $q$-gram threshold (or length) guaranteeing full-sensitivity and opens the door to more specific filtration schemes.
...
Gapped $q$-grams are based on a generalization of $q$-gram similarity (section~\ref{sec:filtering:qgrams-ext}) from substrings to \emph{subsequences} (\ie \emph{non-contiguous} sequences of symbols of another string).
Filtration with gapped $q$-grams thus counts the number of subsequences of length $q$ common to two strings, whose positions are taken from a fixed set $Q$.
...
As in lemma \ref{lemma:qgrams}, the $Q$-gram threshold still depends only on parameters $(m,k)$,
thus I still consider filtration schemes $(Q,t)$ solving $(m,k)$ instances.
However, contrary to $q$-grams, function $\tau_Q$ now provides only a lower bound to the minimum threshold guaranteeing full-sensitivity.
...
\wrt									\\
& \mathbf{x} \in \Bo^m_k				\\
& \mathbf{c} \in \Bo^{m - s(Q) + 1}		\\
& x_i \geq c_j	\quad \quad \forall \, i,j	\\
\end{array}
\end{equation}
