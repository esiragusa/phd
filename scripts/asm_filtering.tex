...
\begin{eqnarray}
E[C] = \sum_{i=1}^{n-q+1}{\Pr[C > 0]} = \frac{n - q + 1}{\sigma^q} \leq \frac{n}{\sigma^q}.
\end{eqnarray}
...
\begin{figure}[t]
\begin{center}
\caption[Filtration with approximate seeds]{Filtration with approximate seeds. A filtration scheme with thresholds $\mathbf{t} = (1,1,1)$ solves $5$-difference. In the illustration, pattern $p$ occurs in text $t$ at edit distance 5. The seed in grey is covered only by one error and thus preserved.}
\label{fig:seeds-apx}
...
Both \cite{Myers1994} and \cite{Navarro2000} carried out involved analysis to estimate the optimal parameterization. \citeauthor{Navarro2000} find out that a number of seeds $s=\Theta(\frac{m}{\log_{\sigma}{n}})$ yields an overall time complexity sublinear for an error rate $\epsilon < 1 - \frac{e}{\sqrt{\sigma}}$.
...
\begin{figure}[h]
\begin{center}
\caption[Filtration with contiguous $q$-grams]{Filtration with contiguous $q$-grams. A filtration scheme $(q,t) = (4,2)$ solves the instance $(25,5)$. In the illustration, pattern $p$ of length 25 occurs in text $t$ at edit distance 5. In this worst-case positioning of the errors, the two grey $q$-grams are preserved.}
...
I denote by a pair $(q,t)$ the filtration scheme counting $q$-grams with threshold $t$.
...
The verification stage then verifies only those parallelograms exceeding threshold $t$.
...
\begin{algorithm}[H]
\Algorithm{ApproximateThreshold}{$Q,m,k$}
\begin{tabular}{ll}
...
\begin{minipage}[t]{.8\textwidth}
\begin{algorithm}[H]
%\Algorithm{CountTranscripts}{$Q,t,m,\epsilon$}
\Algorithm{CountTranscripts}{$Q,m,\epsilon$}
\begin{tabular}{ll}
\textbf{Input}  & $Q$ : $Q$-gram sequence\\
...
\State $T \gets 0$
\Repeat
	\State {$i \gets$ \Call{random}{$\{ 1 \dots C \}$}}
	\State {$\mathbf{x} \gets$ \Call{random}{$\{0,1\}^m$}}
	\ForAll {$j \in Q$}
		\State {$x_{i+j} \gets 1$}
	\EndFor
...
\begin{figure}[b]
\begin{center}
\caption[Filters runtime on $k$-mismatches]{Filters runtime on $k$-mismatches. Pattern length is fixed to 100. Solid lines represent total runtimes, while dashed lines represent filtration times only.}
...
As shown in table \ref{tab:filter:ppv}, I define \emph{true positives} (TP), \emph{false positives} (FP), \emph{true negatives} (TN), or \emph{false negatives} (FN) in terms of the number of verifications $V_f(t)$ produced in the filtration phase and the number of approximate occurrences $C(t)$ of the pattern in the text.
...
\begin{table}[t]
\begin{center}
\caption[Measurement of filtering methods efficiency]{Measurement of filtering methods efficiency. $V_f(t)$ counts the number of verifications, $C(t)$ the number of approximate occurrences, and $|t|$ the text length. Since all considered filtering methods are full-sensitive, the number of false negatives is always 0.}
\begin{tabular}{rcc}
\toprule
  & Positive & Negative\\
\midrule
