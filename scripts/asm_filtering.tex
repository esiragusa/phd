...
{\bf Solution}	&	The number transcripts $\sigma \in \bar{\Bo}^{m}_{k}$ \st $t_{Q}^{m}(\sigma) \geq t$\\
%{\bf Solution}	&	The number of false positive transcripts detected by $(Q,t)$.\\
\end{tabular}
\\
...
\text{}_{k}^{m}(Q,t) = \sum_{\sigma \in {\bar{\Bo}^{m}_{k}}} t_{Q}^{m}(\sigma) \geq t
\end{equation}
...
\begin{minipage}[t]{.8\textwidth}
\begin{algorithm}[H]
\Algorithm{TranscriptsDetected}{$Q,t,m,k$}
\begin{tabular}{ll}
\textbf{Input}  & $Q$ : $Q$-gram sequence\\
