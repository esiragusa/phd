...
%
%The filtration step can be online or offline.
%
...
\citeauthor{Myers1994}, \citeauthor{Navarro2000} carried out involved analysis to estimate the optimal parameterization. \citeauthor{Navarro2000} find out that the optimal number of seeds $\Theta(\frac{m}{\log_{\sigma}{n}})$ yields an overall time complexity sublinear for an error rate $\epsilon < 1 - \frac{e}{\sqrt{\sigma}}$.
...
%I give a counting lemma to derive a full-sensitive threshold.
%Then, I explain how to implement a counting filter.
%Finally, I
...
When the pattern matches approximately a substring of the text, the length of such substring is $m$ for $k$-mismatches, or within $m - k$ and $m + k$ for $k$-differences.
The dot plot representation clarifies this concept: $k$-mismatches occurrences cover one single diagonal of the dot plot, while $k$-difference occurrences are enclosed inside a parallelogram of width $2k+1$.
%Overlapping parallelograms?

An online filtration algorithm scans the text and counts how many $q$-grams of the pattern fall into each parallelograms.
Parallelograms exceeding threshold $\tau_q(m,k)$ are verified with an online method, \eg standard DP.

%QUASAR, SWIFT, Stellar. RazerS.

Alternatively, to speed up the filtration phase, $q$-grams can be counted with the help of an index of the text.
