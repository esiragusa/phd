...
For indexed filters, I use the $q$-gram index for $q=10$ (see \ref{sec:index:qgram}).
As text, I take the \emph{C.~elegans} reference genome (WormBase WS195), \ie a collection of 6 DNA strings of about 100~MB total length.
As patterns, I extrapolated $200k$ DNA sequences of length 100 from an Illumina sequencing run (Sequence Read Archive ID SRR065390).
...
I measure the runtime of the filtration phase only, and then of the filtration plus verification phase.

On $k$-mismatches, exact seeds are the best scheme for $k \leq 7$, while 1-approximate seeds are better for $k \geq 8$.
This is mainly due to the filtration speed of exact seeds 
...
It can be seen that enforcing $t \geq 4$ improves the runtime 
...
\begin{figure}[h]
\begin{center}
\caption[Filters runtime on $k$-differences]{Filters runtime on $k$-differences. Pattern length is fixed to 100. Dashed lines represent filtration time only.}
...
I plot the runtime ratio of the verification versus the filtration phase.



\begin{figure}[h]
\begin{center}
\caption[Ratio on $k$-mismatches]{Ratio on $k$-mismatches. Pattern length is fixed to 100.}
...
I instead measure the positive predictive value (PPV).

\begin{figure}[h]
\begin{center}
\caption[Filters specificity on $k$-mismatches]{Filters specificity on $k$-mismatches. Pattern length is fixed to 100.}
