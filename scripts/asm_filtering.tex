...
This fact motivates me to give integer linear programs (ILPs) and approximate algorithms for the above problems.
...
Pseudo-boolean function $t_{Q}^{m}$ (equation \ref{eq:qgram-pseudo}) has two important properties: it is \emph{monotone non-decreasing} and \emph{supermodular}.
In particular, supermodularity and submodularity are the discrete analogs of concavity and convexity.
...
%\end{equation}
It is immediate to see that function $t_{Q}^{m}$ satisfies all the above inequalities, as by definition it does not contain any term in negative form (see equation \ref{eq:qgram-pseudo}).

\textsc{Optimal Threshold} thus involves the minimization of a supermodular function subject to linear constraints.
...
%\end{tabular}
%\end{problem}
% $f$ is \emph{submodular} if $-f$ is supermodular.
