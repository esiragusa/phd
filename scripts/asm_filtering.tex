...
\begin{align}
%\label{eq:qgram-ilp}
\min \sum T\label{eq:qgram-ilp-sum}\\
\text{subject to}\nonumber				\\
& T \in \Bo^{\Plen - \Span{Q} + 1}\label{eq:qgram-ilp-t}\\
...
Vector $\Transcript$ represents any Hamming transcript: each $x_j$ is subject to an integer linear constrain \st the Hamming distance of $\Transcript$ is within $k$ (constraint~\ref{eq:qgram-ilp-transcript}).
Vector $T$ represents function $\Threshold_{Q}^{m}$: each $\Threshold_i$ indicates the truthfulness of the $i$-th clause of $T_{Q}^{m}$ (constraint~\ref{eq:qgram-ilp-t}) and its sum (objective function~\ref{eq:qgram-ilp-sum}) the value of $\Threshold_{Q}^{m}(\Transcript)$.
...
The solution $T^*$ to the above ILP provides the optimal threshold $\Threshold=\sum T^*$ for a $Q$-gram on instance $(\Plen,k)$.
