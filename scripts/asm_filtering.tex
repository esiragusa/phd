...
To obtain even more specific filtration, \cite{Kucherov2005} propose \emph{$q$-gram families} for approximate string matching.
A $q$-gram family consists of \emph{multiple} gapped $q$-grams used in conjunction.
...
\end{equation}
By definition, $T_{Q}^{m}$ and $T_{\mathbb{F}}^{m}$ are \emph{monotone nondecreasing} boolean functions in \emph{disjunctive normal form} (\emph{DNF}).
Since all monotone boolean functions in DNF are minimal, $T_{Q}^{m}$ and $T_{\mathbb{F}}^{m}$ are \emph{minimal}.
...
\emph{Nondecreasing monotonicity} of functions $t_{Q}^{m}$ and $t_{\mathbb{F}}^{m}$ follow from nondecreasing monotonicity of their boolean counterparts $T_{Q}^{m}$ and $T_{\mathbb{F}}^{m}$. Consequently $\bar{t}_{Q}^{m}$ and $\bar{t}_{\mathbb{F}}^{m}$ are \emph{monotone nonincreasing}.
...
Since super and submodular functions are closed under non-negative linear combination, functions $t_{\mathbb{F}}^{m}$ and $\bar{t}_{\mathbb{F}}^{m}$ are respectively super and submodular.
