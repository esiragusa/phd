...
The simple analysis of section~\ref{sec:intro:filtering} show us that filtration specificity is strongly correlated to the parameter $q$ (seed or $q$-gram length).
...
We have seen in the $q$-gram lemma~\ref{lemma:qgram} that one error covers $q$ consecutive $q$-grams.
...
The idea of approximate seeds is that, to increase the seed length, we can factorize the pattern in less than $k+1$ non-overlapping seeds.
...
Summing up, instead of reducing an approximate search into smaller exact searches, we reduce it into smaller approximate searches.

Problem of exact and approximate seeds filters are that
\begin{inparaenum}[(i)]
...
We now generalize this similarity measure to consider the number of common subsequences of length $q$ whose symbols are taken from a fixed set $Q$ of positions.
...
\begin{definition}
We call $q$-gram shape a finite sequence $Q$ of natural numbers starting with the unit element, \ie $Q \subset \N$ and $1 \in Q$.
We denote by $w(Q)$ the \emph{weight} of $Q$ as the cardinality of the set $|Q|$, and by $s(Q)$ the \emph{span} of $Q$ as the maximum element of the set $\max{Q}$.
\end{definition}

In literature, $q$-gram shapes are visualized as words over the alphabet $\{1,*\}$ or $\{\#,-\}$.
The representation of the shape $Q$ is the word $w \in \{1,*\}^{s(Q)}$ such that $j \in Q$ iff $w_j=1$, or equivalently iff $w_j=\#$.
We adopt the former notation.
Figure~\ref{fig:qgram-shape} shows an example of $q$-gram shape.
...
Note that, as in the $q$-gram lemma, the threshold depends only on $Q$ and $(m,k)$.
...
A seed $Q$ \emph{occurs} at position $i$ in a similarity $\sigma$ iff $\forall j \in Q$ $\sigma_{i+j}=1$.
Fixed a $q$-gram threshold $t$, the shape $Q$ detects $\sigma$ iff it occurs at least $t$ times in $\sigma$.


\subsubsection{Boolean functions}

Let $T_{Q}^{m}: \Bo^m \rightarrow \Bo$ denote a \emph{boolean function} such that $T_{Q}^{m}(\sigma)$ is true iff seed $Q$ occurs at least one time in a similarity $\sigma$ of length $m$.
...
We define an analogous boolean function for a seed family $F$ as the disjunction
\begin{equation}
...
\end{equation}
Here $t_{Q}^{m}(\sigma)$ counts how many times seed $Q$ occurs in a similarity $\sigma$ of length $m$.
It is useful to define the complementary function $\bar{t}_{Q}^{m}$, counting how many times a seed $Q$ does not occur in a similarity $\sigma$, as
\begin{equation}
\label{eq:qgram-pseudoneg}
\bar{t}_{Q}^{m}(\sigma) = m - s(Q) + 1 - t_{Q}^{m}(\sigma)
\end{equation}
Analogously, we define a pseudo-boolean function for a seed family $F$
\begin{equation}
...
\paragraph{}
\begin{tabular}{rl}
{\bf Instance}	&	A seed $Q$, two integers $m,k$ with $0 < k < m$. \\
...
\subsubsection{Our FPRAS}

%Recalling seed pseudo-boolean functions \eqref{sec:seed_pbf}.
...
\paragraph{}
\begin{tabular}{rl}
{\bf Instance}	&	A seed $Q$, two integers $m,k$ such that $0 < k < m$.\\
{\bf Solution}	&	The largest integer $t^*$ such that \textsc{Non Detection} for $(Q,t^*),m,k$ answers \emph{no}.\\
\end{tabular}
\\

Recalling seed pseudo-boolean functions \eqref{sec:seed_pbf}, we can define the optimal threshold problem as the minimization of a supermodular function subject to linear constraints
\begin{equation}
\begin{array}{ll}
\min & t_{Q}^{m} (\sigma)			\\
w.r.t.								\\
...
\citeauthor{Kucherov2005} give an extension for seed families in \citep{Kucherov2005}.
...
\paragraph{}
\begin{tabular}{rl}
{\bf Instance}	&	A seed $Q$, an integer $m$.\\
...
\begin{tabular}{rl}
{\bf Instance}	&	A seed $Q$, two integers $m,k$ such that $0 < k < m$.\\
{\bf Solution}	&	The number of false positives of $Q$.\\
\end{tabular}
\\
...
{\bf Solution}	&	A seed $Q$ that detects all similarities of $S$.\\
{\bf Measure}	&	The weight of $Q$, $w(Q)$.\\
\end{tabular}
\\

Given a set of similarities, find the seed of maximum weight detecting all similarities.
