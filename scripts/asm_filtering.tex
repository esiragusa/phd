...
\end{tabular}
\end{table}
%\label{enum:qgram-distance} Which is the minimum distance $k^*$ for which $(m,k^*)$ is not solved by $(Q,t)$?
...
\cite{Nicolas2005} considered the decision problem \textsc{Non Detection} associated to \textsc{Optimal Threshold}.
They show that \textsc{Non Detection} is \emph{strongly} NP-complete by performing an indirect reduction from \textsc{Exact Cover by 3-Sets}.
...
This fact motivates me to give integer linear programs (ILPs) or approximate solutions to \textsc{Optimal Threshold}.

%\cite{Burkhardt2001} considered also question \label{enum:qgram-fp}.

%\item Which $Q$-gram provides the most specific filtration?

\subsection{Non detection}
\label{sub:qgram-non-detection}

In order to formally define \textsc{Non Detection}, I introduce a simple characteristic function indicating which transcripts pass through a $Q$-gram filtration scheme.
...
Let $T_{Q}^{m}: \Bo^m \rightarrow \Bo$ denote the boolean function such that $T_{Q}^{m}(\mathbf{x})$ is true iff the $Q$-gram occurs in a transcript $\mathbf{x}$ of length $m$.
...
According to definition \ref{def:qgram-detection}, filtration scheme $(Q,t)$ detects $\mathbf{x}$ iff $\mathbf{x}$ satisfies at least $t$ clauses of $T_{Q}^{m}$.
The formal definition of \textsc{Non Detection} follows.
...
\begin{problem}[\textsc{Non Detection}]
\begin{tabular}{rl}
...
\begin{tabular}{rl}
{\bf Instance}	&	A $Q$-gram, an $(m,k)$ instance.\\
{\bf Solution}	&	$\min \, t_{Q}^{m}(\mathbf{x})$ \wrt $\mathbf{x} \in \Bo^m_k$
\end{tabular}
\end{problem}
...
%I reduce complementary \textsc{optimal threshold} to \textsc{maximum coverage} \citep{Vazirani2001} and solve it with the following ILP
...
Vector $\mathbf{x}$ represents the hamming transcript; each $x_i$ is subject to an integer linear constrain \st the hamming distance of $\mathbf{x}$ is within $k$.
Vector $\mathbf{c}$ represents the function $T_{Q}^{m}$; each $c_j$ indicates the truthfulness of the $j$-th clause in $T_{Q}^{m}$.
The set of inequalities binds the truthfulness of each clause $c_j$ to its corresponding $x_i$ in the transcript.
The exact solution to \textsc{Optimal Threshold} is derived as $t^* = m - s(Q) + 1 - \sum c^*$ from the ILP solution $c^*$.

\subsubsection{Approximate solution}

Pseudo-boolean function \ref{eq:qgram-pseudo} has particular properties in respect to monotonicity and modularity.
\emph{Supermodularity} and \emph{submodularity} are the discrete analogs of concavity and convexity.
These properties allow to compute in polynomial time approximate solutions to optimization problems subject to certain constrains.
...
\end{equation}
This latter definition is evident for $t_{Q}^{m}$, as definition \ref{eq:qgram-pseudo} does not contain any negative term.
Thus, function $t_{Q}^{m}$ is supermodular.
The non-decreasing monotonicity of function $t_{Q}^{m}$ immediately follows from the non-decreasing monotonicity of its boolean counterpart $T_{Q}^{m}$.

%I define the complementary function $\bar{t}_{Q}^{m}$, counting how many times a $Q$-gram does not occur in a transcript $\mathbf{x}$, as
%\begin{equation}
%\label{eq:qgram-pseudoneg}
%\bar{t}_{Q}^{m}(\mathbf{x}) = m - s(Q) + 1 - t_{Q}^{m}(\mathbf{x})
%\end{equation}
%Consequently, the complementary function $\bar{t}_{Q}^{m}$ is monotone nonincreasing and submodular.
%
%I consider the \emph{complementary} \textsc{Optimal Threshold} problem as the maximization of a submodular function subject to linear constraints:
%\begin{problem}[\textsc{Optimal Threshold}]
%\begin{tabular}{rl}
%{\bf Instance}	&	A $Q$-gram, an $(m,k)$ instance.\\
%{\bf Solution}	&	$\max \, \bar{t}_{Q}^{m}(\mathbf{x})$ \wrt $\mathbf{x} \in \bar{\Bo}^m_k$
%\end{tabular}
%\end{problem}

\textsc{Optimal Threshold} involves the minimization of a supermodular function subject to linear constraints.
...
The same \emph{absolute error} applies to \textsc{Optimal Threshold}, but not the relative error, thus optimal threshold cannot be said to be in APX.
...
Moreover, $T_{Q}^{m}$ is \emph{monotone non-decreasing}.
