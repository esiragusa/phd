...
\begin{figure}[t]
\begin{center}
\caption[Filtration with approximate seeds]{Filtration with approximate seeds. A filtration scheme with thresholds $\Scheme = (1,1,1)$ solves $5$-difference. In the illustration, pattern $\Pattern$ occurs in text $\Text$ at edit distance 5. The seed in grey is covered only by one error and thus preserved.}
\label{fig:seeds-apx}
...
A seeds filtration scheme is an integer vector $\Scheme = (t_{\At{1}}, \dots, t_{\AtEnd{s}})$, where integer $\Threshold_i \in \N_0$ represents the threshold assigned to the $i$-th seed.
\end{definition}

\begin{lemma}
\label{lemma:apx-scheme}
Any filtration scheme $\Scheme = (t_{\At{1}}, \dots, t_{\AtEnd{s}})$ \st
\begin{eqnarray}
...
Almost all of them focus on lossy filtration for homology search, rather than full-sensitive filtration for approximate string matching.
...
The value $1 + \max\,Q$ is named \emph{span} and it is indicated by $\Span{Q}$.
\end{definition}
...
Conversely, such a choice is non-trivial for $Q$-grams, as the search space of $Q$ is exponentially large in the span $\Span{Q}$ and a full-sensitivity threshold for arbitrary $Q$-grams is hard to compute.
...
I give an \emph{integer linear program} (ILP) that solves exactly \textsc{Optimal Threshold} and it is orders of magnitude quicker than the DP algorithm from \citep{Burkhardt2001} (see table~\ref{tab:threshold}).
...
\label{problem:opt-threshold}
\begin{tabular}{rl}
{\bf Instance}	&	A $Q$-gram, an $(\Plen,k)$ instance.\\
{\bf Solution}	&	$\min \, \Threshold_{Q}^{\Plen}(\Transcript)$ subject to $\Transcript \in \Bo^{\Plen}_k$
\end{tabular}
\end{problem}

\subsubsection{Exact solution}

The key to formulate problem~\ref{problem:opt-threshold} as an ILP is to rewrite function $t_{Q}^{m}$ as a linear combination of boolean variables.
I start by representing function $\Threshold_{Q}^{\Plen}$ using a boolean vector $T$ of size $\Plen - \Span{Q} + 1$, where each $\Threshold_i$ indicates the truthfulness of the $i$-th clause (constraint~\ref{eq:qgram-inlp-t}).
In addition, I represent any Hamming transcript by a boolean vector $\Transcript$: each $x_j$ is subject to an integer linear constraint \st the Hamming distance of $\Transcript$ is within $k$ (constraint~\ref{eq:qgram-inlp-transcript}).
This leads to the following non-linear formulation
\begin{align}
\hspace{1cm}\min \sum t_i\label{eq:qgram-inlp-sum}\\
\hspace{1cm}\text{subject to }&T \in \Bo^{\Plen - \Span{Q} + 1}\label{eq:qgram-inlp-t}\\
&\Transcript \in \Bo^{\Plen}_k\label{eq:qgram-inlp-transcript}\\
&t_i \geq \prod_{j \in Q} x_{i+j} &\hspace{-8cm}\forall \, \At{1} \leq i \leq \Plen-\Span{Q}\label{eq:qgram-inlp-constr}
\end{align}
Using de Morgan's law, I rewrite inequality~\ref{eq:qgram-inlp-constr} as
\begin{eqnarray}
\overline{t_i} \leq \sum_{j \in Q}{\overline{x_{i+j}}}.
\end{eqnarray}
Thus, for convenience, I consider vectors $T$ and $X$ in negated form; this involves negating boolean constraints \ref{eq:qgram-inlp-transcript}--\ref{eq:qgram-inlp-t} and maximizing objective function \ref{eq:qgram-inlp-sum}.

The following ILP solves \textsc{Optimal Threshold}:
\begin{align}
\hspace{1cm}\max \sum \overline{t_i}\label{eq:qgram-ilp-sum}\\
\hspace{1cm}\text{subject to }& \overline{T} \in \Bo^{\Plen - \Span{Q} + 1}\label{eq:qgram-ilp-t}\\
& \overline{\Transcript} \in \Bo^{\Plen}_{\Plen-k}\label{eq:qgram-ilp-transcript}\\
&  \overline{t_i} \leq \sum_{j \in Q}{\overline{x_{i+j}}} & \hspace{-8cm} \forall \, \At{1} \leq i \leq \Plen-\Span{Q}\label{eq:qgram-ilp-constr}
\end{align}
The solution $T^*$ to the above ILP provides the optimal threshold $\sum t_i^*$ for a $Q$-gram on instance $(\Plen,k)$.
Table~\ref{tab:threshold} shows the performance of ILP~\ref{eq:qgram-ilp-sum} on selected problem instances.
...
A pseudo-boolean function $f : \Bo^{\Plen} \rightarrow \Z$ is monotone non-decreasing iff 
\begin{equation}
\label{eq:monotone-non-decreasing}
f(Y) \leq f(Y \vee Z) \quad \forall \, Y, Z \in \Bo^{\Plen}
\end{equation}
and it is \emph{supermodular} iff 
\begin{equation}
\label{eq:supermodularity}
f(Y) + f(Z) \leq f(Y \vee Z) + f(Y \wedge Z) \quad \forall \, Y, Z \in \Bo^{\Plen}.
\end{equation}
%or, equivalently, if all its second order derivatives are non-negative, \ie
%\begin{equation}
%\label{eq:supermodularity-partial}
%\frac{\partial^2 f}{\partial z_i \partial z_j}(Z) \geq 0 \quad \forall \, Z \in \Bo^{\Plen} \text{ and } \forall \, 1 \leq i \leq j \leq m
%\end{equation}
...
\end{tabular}
\begin{algorithmic}[1]
\State {$\Transcript \gets \mathbf{1}^{\Plen}$}
\While {$k > 0$}
	\State {i \gets $ \argmax_{j=1}^{m} \frac{\partial t^{\Plen}_Q(\Transcript)}{\partial x_j}$}
%	\State {i \gets $ \argmax_{j=1}^{m} \frac{\partial \bar{t}^{\Plen}_Q(\Transcript)}{\partial x_j}$}
	\State {$x_i \gets 0$}
	\State {$k \gets k - 1$}
\EndWhile
\State \Return $\Threshold^{\Plen}_Q(\Transcript)$
%\State \Return $\Plen - \Span{Q} + 1 - \bar{t}^{\Plen}_Q(\Transcript)$
\end{algorithmic}
...
%and later discuss its generalization to $(Q,t)$.
...
One might think of a randomized algorithm that estimates $\left| \Detected \right|$ by repeatedly sampling $\Transcript \in \Bo^{m}$ and counting whether $\Transcript \in \Detected$ too.
...
\begin{eqnarray}
%\Pr \left[ \Transcript \leftarrow \mathbb{M} \right] = 
...
Algorithm~\ref{alg:qgram-specificity} counts $\Transcript$ (line 10) with weight
\begin{eqnarray}
...
\DoRepeat
	...
	\State {$\Transcript \gets$ \Call{random}{$\{0,1\}^{\Plen}$}}
	\ForAll {$j \in Q$}
		\State {$x_{i+j} \gets 1$}
	\EndFor
...
For offline filters, I use the $q$-gram index with $q=10$ (see \ref{sec:index:qgram}).
