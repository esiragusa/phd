...
\begin{figure}[h]
\begin{center}
\caption[Filtration with approximate seeds]{Filtration with approximate seeds. A filtration scheme with thresholds $\mathbf{k} = (1,1,1)$ solves $5$-difference. In the example, pattern $p$ occurs in text $t$ at edit distance 5. The left and right seeds are covered by two errors, but the central seed is covered only by one error and thus preserved.}
\label{fig:seeds-apx}
...
\begin{definition}
A seeds filtration scheme is an integer vector $\mathbf{k} = (k_1, \dots, k_s)$, where integer $k_i \in \N_0$ represents the threshold assigned to the $i$-th seed.
\end{definition}

\begin{lemma}
\label{lemma:apx-scheme}
Any filtration scheme $\mathbf{k} = (k_1, \dots, k_s)$ \st
\begin{eqnarray}
s + \sum_{i=1}^s k_i > k
\end{eqnarray}
...
I consider any Hamming distance transcript as a $m$-dimensional vector $\mathbf{x} = (x_1, x_2, \dots x_m)$ over $\Bo = \{ 0, 1 \}$.
...
A $(Q,t)$ filtration scheme \emph{detects} $\mathbf{x}$ iff the $Q$-gram occurs at least $t$ times in $\mathbf{x}$.
\end{definition}
