...
I consider a so called $k$-differences , and say that a \emph{full-sensitive} filtration scheme \emph{solves} a $(m,k)$-differences instance.

\begin{definition}
A seeds filtration scheme is a pair of integer vectors $(\mathbb{s},\mathbb{k})$, where
\begin{eqnarray}
\mathbb{s} \in \N^s\\
\mathbb{k} \in \N_0^s\\
\sum \mathbb{s} \leq m
\end{eqnarray}
The integer $s_i \in \mathbb{s}$ represents the length of the $i$-th seed.
The first seed starts at position $1$ in the pattern, while the $i$-th seed starts at position $\sum_{j=1}^{i-1}{s_j}$.
The integer $k_i \in \mathbb{k}$ represents the threshold assigned to the $i$-th seed.
\end{definition}

\begin{lemma}
\label{lemma:apx-scheme}
Given a $(m,k)$-differences instance, any filtration scheme $(\mathbb{s},\mathbb{k})$, \st
\begin{eqnarray}
\sum \mathbb{k} \geq k - |\mathbb{s}|
\end{eqnarray}
is full-sensitive.
\end{lemma}

\begin{example}
The following $(\mathbb{s},\mathbb{k})$ filtration schemes solve $(100,4)$-differences:

$\mathbb{s}=(20,20,20,20,20)$, $\mathbb{k}=(0,0,0,0,0)$

$\mathbb{s}=(33,33,33)$, $\mathbb{k}=(1,1,0)$
\end{example}
...
For this reason, I call any pair $(q,t)$ solving a $(m,k)$ instance a valid $q$-gram filtration scheme.
...
Which is the maximum $k^*$ for which a $Q$-gram guarantees full-sensitivity?
...
How many false positives a filtration scheme $(Q,t)$ produces?

\subsubsection{Problem definition}

\paragraph{}
\begin{tabular}{rl}
{\bf Instance}	&	A $Q$-gram and a threshold $t$, two integers $m,k$ such that $0 < k < m$.\\
{\bf Solution}	&	The number of false positives produced by the filtration scheme $(Q,t)$.\\
\end{tabular}
\\
...
TODO.
I experimentally evaluate filtration efficiency.
