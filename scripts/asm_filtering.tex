...
{\bf Solution}	&	$| \{ \mathbf{x} \in \Bo^{\Plen} : t_{Q}^{\Plen}(\mathbf{x}) \geq t \} |$
\end{tabular}
\end{problem}

\subsubsection{Approximate solution}

I propose a randomized algorithm to estimate the number of transcripts detected by a filtration scheme of the type $(Q,1)$ and later discuss its generalization to $(Q,t)$.
The number of transcripts detected by $(Q,1)$ coincides with the number of true assignments to the boolean function $T_{Q}^{m}$,
%, \ie $| \{ \mathbf{x} \in \Bo^{m} : T_{Q}^{m}(\mathbf{x}) \} |$.
which is in \emph{disjunctive normal form} (\emph{DNF}) by definition (see equation~\ref{eq:qgram-bool}).
...
I denote with $\mathbb{D}_i$, for $\At{1} \leq i \leq \Plen-\Span{Q}$, the set of transcripts $\mathbf{x}$ detected by the $Q$-gram starting at position $i$
\begin{eqnarray}
\mathbb{D}_i = \left\{ \mathbf{x} \in \Bo^{\Plen} : \bigwedge_{j \in Q} x_{i+j} \right\} 
\end{eqnarray}
which has cardinality 
\begin{eqnarray}
\label{eq:card-di}
\left| \mathbb{D}_i \right| = 2^{\Plen-\Weight{Q}}
\end{eqnarray}
...
\begin{eqnarray}
\left| \left\{ \mathbf{x} \in \Bo^{m} : T_{Q}^{m}(\mathbf{x}) \right\} \right| = \left| \bigcup_{i=\At{1}}^{\AtEnd{\Plen}}{\mathbb{D}_i} \right|
\end{eqnarray}
which is less than the sum of all $|\mathbb{D}_i|$ from equation~\ref{eq:card-di} and non-trivial to compute in general.

Recall that $t_{Q}^{\Plen}(\mathbf{x})$ indicates the number of clauses of $T_{Q}^{m}$ which are satisfied by $\mathbf{x}$.
I denote by $\mathbb{M}$ the multi-set
\begin{eqnarray}
\mathbb{M} = \left\{ \left( \mathbf{x}, t_{Q}^{m}(\mathbf{x}) \right) : \mathbf{x} \in \mathbb{D}_i \text{ for } \At{1} \leq i \leq \Plen-\Span{Q} \right\}
\end{eqnarray}
which has cardinality 
\begin{eqnarray}
\left| \mathbb{M} \right| = \sum_{i=\At{1}}^{\AtEnd{\Plen}}{\left| \mathbb{D}_i \right| } = \left( \Plen - \Span{Q} + 1 \right) \cdot 2^{\Plen-\Weight{Q}}.
\end{eqnarray}

Algorithm~\ref{alg:qgram-specificity} picks $\mathbf{x} \in \mathbb{M}$ with probability
\begin{eqnarray}
\frac{t_{Q}^{m}(\mathbf{x})}{\left| \mathbb{M} \right|}
\end{eqnarray}
by picking (line 6) $\mathbb{D}_i$ with probability
\begin{eqnarray}
\frac{\left| \mathbb{D}_i \right|}{\left| \mathbb{M} \right|} = \frac{1}{\Plen - \Span{Q} + 1}
\end{eqnarray}
and then (lines 7--9) picking $\mathbf{x} \in \mathbb{D}_i$ with probability
\begin{eqnarray}
\frac{1}{\left| \mathbb{D}_i \right|} = \frac{1}{2^{\Plen-\Weight{Q}}}.
\end{eqnarray}
It estimates the mean value of
\begin{eqnarray}
\frac{\left| \mathbb{M} \right|}{t_{Q}^{m}(\mathbf{x})}
\end{eqnarray}
by sampling $\mathbb{M}$ for $4 \cdot \left( \Plen - \Span{Q} + 1 \right) / \epsilon^2$ times (lines 3, 11).
