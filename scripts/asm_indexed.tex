...
Thus, while traversing the trie, we maintain the interval $[l,r]$ of the current node $\Tn$.
In addition, in order to answer \textsc{depth}$(\Tn)$, we also maintain the depth of the current node.
...
The label of the edge entering any suffix trie node at depth $d$ is defined by the symbol at position $d$ in any suffix $\Sc_{A[i]}$ with $i \in [l,r]$.
...
\begin{algorithmic}[1]
\Procedure{isRoot}{$\Tn$}
	\State \Return \Call{depth}{$\Tn$} = 0 %$[l,r] = [1,n]$
\EndProcedure
\end{algorithmic}
\end{minipage}
\begin{minipage}[t]{.5\textwidth}
...
\begin{algorithmic}[1]
\Procedure{isLeaf}{$\Tn$}
	\State \Return $A[r]$ + \Call{depth}{$\Tn$} = $n^{A[r]}$
\EndProcedure
\end{algorithmic}
\end{minipage}
\end{algorithm}
...
\begin{algorithmic}[1]
\Procedure{label}{$\Tn$}
	\State \Return $\Sc_{A[l] + \Call{depth}{\Tn}}$
\EndProcedure
\end{algorithmic}
\end{minipage}
\begin{minipage}[t]{.5\textwidth}
\label{alg:sa-label}
\begin{algorithmic}[1]
\Procedure{getOccurrences}{$\Tn$}
	\State \Return $A[l \dots r]$
\EndProcedure
\end{algorithmic}
\end{minipage}
\end{algorithm}
...
\Procedure{L}{$x,p$}
	...
	\State {$e \gets r$}
	\While {$b < e$}
		\State {$i \gets \lfloor \frac{b+e}{2} \rfloor$}
		\If {$\Sc_{A[i] + \Call{depth}{\Tn} \dots n} <_{lex} p$}
			\State {$b \gets i+1$}
		\Else
			\State {$e \gets i$}
		\EndIf
...
\Procedure{R}{$x,p$}
	...
	\While {$b < e$}
		\State {$i \gets \lfloor \frac{b+e}{2} \rfloor$}
		\If {$\Sc_{A[i] + \Call{depth}{\Tn} \dots n} \leq_{lex} p$}
			\State {$b \gets i+1$}
		\Else
			\State {$e \gets i$}
		\EndIf
...
\Procedure{goDown}{$\Tn$}
	\If {\Call{isLeaf}{$\Tn$}}
		\State \Return \False
	\EndIf
	\State {$\Inc \Call{depth}{\Tn}$}
%	\While {$|\Sc_{A[l] \dots n^{A[l]}}| \leq \Call{depth}{\Tn}$}
%		\State \Inc l
%%		\If {}
%%			\Return \False
%%		\EndIf
%	\EndWhile
	\State {$c_l \gets \Sc_{A[l] + \Call{depth}{\Tn}}$}
	\State {$c_r \gets \Sc_{A[r] + \Call{depth}{\Tn}}$}
	\If {$c_l \neq c_r$}
...
\Procedure{goRight}{$\Tn$}
	\If {\Call{isRoot}{$\Tn$}}
		\State \Return \False
	\EndIf
	\State {$c_l \gets \Sc_{A[l] + \Call{depth}{\Tn} - 1}$}
	\State {$c_r \gets \Sc_{A[r] + \Call{depth}{\Tn} - 1}$}
	\If {$c_l \neq c_r$}
		\State {$l \gets r$}
		\State {$r \gets$ \Call{R}{$l,r,c_l$}}
	\EndIf
...
\Procedure{goDown}{$\Tn,c$}
	\State {$l \gets$ \Call{L}{$l,r,c$}}
	\State {$r \gets$ \Call{R}{$l,r,c$}}
	\State \Return $l < r$
\EndProcedure
\end{algorithmic}
\end{minipage}
...
\If {\Call{goDown}{$\Tn$,p}}
	\State \Report \Call{getOccurrences}{$\Tn$}
\EndIf
\EndProcedure
\end{algorithmic}
\end{minipage}
...
In addition, while traversing the trie, we maintain the interval $[h_l,h_r]$ of the current node $\Tn$ and, in order to answer \textsc{label}$(\Tn)$, we also maintain the label of the edge entering the current node.
We redefine \textsc{isRoot} and \textsc{isLeaf}.

\begin{algorithm}[h!]
\begin{minipage}[t]{.5\textwidth}
\label{alg:qgram-isroot}
\begin{algorithmic}[1]
\Procedure{isRoot}{$\Tn$}
	\State \Return $[l,r] = [1,n]$ \And $[h_l, h_r] = [1, \sigma^q]$
\EndProcedure
\end{algorithmic}
\end{minipage}
\begin{minipage}[t]{.5\textwidth}
\label{alg:qgram-isleaf}
\begin{algorithmic}[1]
\Procedure{isLeaf}{$\Tn$}
	\State \Return $l \geq r$ \Or $h_l \geq h_r$
\EndProcedure
\end{algorithmic}
\end{minipage}
\end{algorithm}
...
%\begin{algorithm}[h!]
\begin{minipage}[t]{.5\textwidth}
\label{alg:qgram-godownp}
\begin{algorithmic}[1]
\Procedure{goDown}{$\Tn,p$}
	\If {\Call{isLeaf}{$\Tn$}}
		\State \Return \False
	\EndIf
	\State \Return $l < r$
