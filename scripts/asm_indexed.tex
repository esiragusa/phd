...
\begin{definition}
The \emph{suffix array} of a padded string $\String$ of length $\Slen$ is an array $\Sarray$ containing a permutation of the interval $\Interval{1}{\Slen}$, \st $\Suffix{\String}{\SubscriptNV{\Sarray}{i}} <_{lex} \Suffix{\String}{\SubscriptNV{\Sarray}{i+1}}$ for all $1 \leq i < \Slen$.
\end{definition}
...
The label of any edge entering some internal node at depth $d$ is defined by the $d$-th symbol within the corresponding suffix $\Suffix{\Strings}{\SubscriptNV{\Sarray}{i}}$ with $i \in \Interval{l}{r}$.
...
\begin{figure}[b!]
\begin{center}
\caption[Example of $q$-gram index]{$2$-Gram index of the string {\ttfamily ANANAS\$} over the alphabet $\Sigma = \{ A, N, S \}$. The example shows the lookup of the $2$-gram {\ttfamily NA}. The hash value $h(\text{\ttfamily NA})=4$ addresses two lookups in $\SubscriptNV{D}{4}$ and $\SubscriptNV{D}{5}$, that in turn provide the range $\Interval{5}{6}$ in $\Sarray$.}
\label{fig:qgram}
\ttfamily
...
The hash function $h$ allows to store in and retrieve from $D$ the left SA interval returned by algorithm~\ref{alg:sa-lower} for each $q$-gram, \ie $p \in \Sigma^q$, $\SubscriptNV{D}{h(p)} = \FunctionCall{L}{1,\Slen,p}$.
Note that the right interval returned by algorithm~\ref{alg:sa-upper} is equivalent to the left interval of the lexicographical successor $q$-gram and therefore available in $\SubscriptNV{D}{h(p)+1}$.
...
\label{obs:lf-a}
\citep{Burrows1994}
For all $i \in \Interval{1}{\Slen}$, the character $\Subscript{\Last}{i}$ precedes the character $\Subscript{\First}{i}$ in the original string $\String$.% (except when $\Subscript{\Last}{i} = \$$);
\end{observation}
\begin{observation}
\label{obs:lf-b}
\citep{Burrows1994}
...
The $j$-th entry of $R$ provides a summary of the number of occurrences of the bit $1$ in $\String$ before position $j b$, \ie $\SubscriptNV{R}{1} = 0$ and $\SubscriptNV{R}{j} = \rank_1(\String, j b - 1)$ for any $j > 1$.
...
If $b$ and $e$ are the first and last position in $\Last$ within $\Interval{b_v}{e_v}$ such that $\Subscript{\Last}{b} = c$ and $\Subscript{\Last}{e} = c$, then $b_w = LF(b)$ and $e_w = LF(e)$.
