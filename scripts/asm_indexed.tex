...
\begin{definition}
The \emph{suffix array} of a padded string $\String$ of length $\Slen$ is an array $\Sarray$ containing a permutation of the interval $\Interval{1}{\Slen}$, \st $\Suffix{\String}{\Sarray[i]} <_{lex} \Suffix{\String}{\Sarray[i+1]}$ for all $1 \leq i < \Slen$.
\end{definition}
...
Pairs are ordered \st $\Suffix{\Strings}{\Sarray[i]} <_{lex} \Suffix{\Strings}{\Sarray[i+1]}$ for all $1 \leq i < \Slen$.
\end{definition}
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ 1, \Slen, 0\}$;
%\item \FunctionCall{isRoot}{\StrieNode} returns true iff $d = 0$;
\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\Sarray[\StrieNode.r] + \StrieNode.d = \Slen_{\Sarray[\StrieNode.r]}$;
\item \FunctionCall{label}{\StrieNode} returns $\Strings_{\Sarray[\StrieNode.l] + \StrieNode.d}$;
\item \FunctionCall{occurrences}{\StrieNode} returns $\Substring{\Sarray}{\StrieNode.l}{\StrieNode.r}$.
\end{itemize}
...
\begin{figure}[b!]
\begin{center}
\caption[Example of $q$-gram index]{$2$-Gram index of the string {\ttfamily ANANAS\$} over the alphabet $\Sigma = \{ A, N, S \}$. The example shows the lookup of the $2$-gram {\ttfamily NA}. The hash value $h(\text{\ttfamily NA})=4$ addresses two lookups in $D[4]$ and $D[5]$, that in turn provide the range $\Interval{5}{6}$ in $\Sarray$.}
\label{fig:qgram}
\ttfamily
\begin{tabular}{ccccccl}
$\Pattern$ & $h(\Pattern)$ & $D[h(\Pattern)]$ & \phantom{-} & $i$ & $\Sarray[i]$ & $\Suffix{\String}{\Sarray[i]}$\\
\midrule
AA & 1 & 2 & & 1 & 7 & \$\\
AN & 2 & 2 & & 2 & 1 & ANANAS\$\\
AS & 3 & 4 & & 3 & 3 & ANAS\$\\
...
The hash function $h$ allows to store in and retrieve from $D$ the left SA interval returned by algorithm~\ref{alg:sa-lower} for each $q$-gram, \ie $p \in \Sigma^q$, $D[h(p)] = \FunctionCall{L}{1,\Slen,p}$.
...
In addition, I maintain the interval $\Interval{l_h}{r_h}$ in $D$ and, in order to answer \FunctionCall{label}{\StrieNode}, the label $e$ of the edge entering the current node.
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ 1, \Slen,0, 1, \sigma^q, \epsilon \}$;
...
Algorithms \ref{alg:qgram-lower} and \ref{alg:qgram-upper} respectively show functions \FunctionName{L} and \FunctionName{R} using the directory $D$ instead of $\Sarray$.
...
Note how the cyclic shifts matrix is related to the suffix array $\Sarray$ of $\String$: the $i$-th cyclic shift is $\Substring{\String}{\Sarray[i]}{\Slen} \Prefix{\String}{\Sarray[i]-1}$.
...
\label{obs:lf-a}
\citep{Burrows1994}
For all $i \in \Interval{1}{\Slen}$, the character $\Last_i$ precedes the character $\First_i$ in the original string $\String$.% (except when $\Last_i = \$$);
\end{observation}
\begin{observation}
\label{obs:lf-b}
\citep{Burrows1994}
...
These observation are evident, indeed $\First = \Subscript{\String}{\Sarray[i]}$ and $\Last = \Subscript{\String}{\Sarray[i]-1}$ (see figure~\ref{fig:lf_psi}).
Given the two above observations, \cite{Ferragina2000} define the permutation $LF$ as:
\begin{eqnarray}
LF(i) = C(\Last_i) + Occ(\Last_i, i)
\end{eqnarray}
...
\begin{figure}[t]
\begin{center}
\caption[Example of binary rank dictionaries]{Binary rank dictionaries (RDs) of the string $\String=$ {\ttfamily 010101100100}. (\subref{fig:rd1}) One-level RD with $b=4$; in the example, $\rank_1(\String, 6) = R[2] + \rank_1(s_{5 \dots 8}, 2) = 3$. (\subref{fig:rd2}) Two-levels RD with $b=2$ (note that $R$ is now different from $b=4$); in the example, $\rank_1(\String, 6) = R^2[2] + R[3] + \rank_1(s_{5 \dots 8}, 1) = 3$.}
...
\begin{figure}[t]
\begin{center}
\caption[Example of one-level DNA rank dictionary]{One-level DNA rank dictionary of the string $\String=$ {\ttfamily CTCGCA} with $b=2$. In the example, $rank_{\text{C}}(\String,4)= R_{\sigma}[2,2] + rank_{\text{C}}(s_{3 \dots 4}, 1)=2.$}
\label{fig:rd_dna}
\ttfamily
\begin{tabular}{rcr}
...
Any non-leaf node $v$ represents some subset of symbols $\Sigma_v$ whose lexicographic rank is in range $\Interval{i}{j}$ \ie $\Sigma_v = \{ c \in \Sigma : \rho(c) \in \Interval{i}{j} \}$, its left child $\Last$ represents the subset $\Sigma_l$ of symbols in range $\Interval{i}{\frac{i+j}{2}}$ while its right child $r$ represents in $\Sigma_r$ those in range $\Interval{\frac{i+j}{2}+1}{j}$.
...
I represent the current node $\StrieNode$ by the elements $\{ l, r, e \}$, where $\Interval{l}{r}$ represents the current suffix array interval and $e$ is the label of the edge entering the current node.
Therefore, I define the following node operations:
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ 1, \Slen, \epsilon \}$;
%\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\StrieNode.r \in \mathbb{I}$;
\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\StrieNode.\Last_r = \$$;
\item \FunctionCall{label}{\StrieNode} returns $\StrieNode.e$.
\end{itemize}
%In order to answer \FunctionName{isLeaf}, I consider the set $\mathbb{I}$ of positions of the terminator symbols $\String^i$ in the BWT.

The traversal easily goes from the root node to its child node labeled by $\StrieChar$: it suffices to derive the interval $\Interval{C(c)}{C(c+1)}$.
Suppose the traversal is on an arbitrary node $v$ of known interval $\Interval{b_v}{e_v}$ \st the path from the root to $v$ spells the substring $s_v$.
Now, the traversal goes down to a child node $w$ of unknown interval $\Interval{b_w}{e_w}$ \st the path from the root to $w$ spells $\StrieChar \cdot s_v$ for some $\StrieChar \in \Sigma$.
The known interval $\Interval{b_v}{e_v}$ contains all prefixes of $\bar{\Strings}$ ending with $s_v$, \ie all suffixes of $\Strings$ starting with $s_v$, while the unknown interval $\Interval{b_w}{e_w}$ contains all prefixes of $\bar{\Strings}$ ending with $\StrieChar \cdot s_v$, \ie all suffixes of $\Strings$ starting with $s_v \cdot c$.
...
If $b$ and $e$ are the first and last position in $\Last$ within $\Interval{b_v}{e_v}$ such that $\Last_b = c$ and $\Last_e = c$, then $b_w = LF(b)$ and $e_w = LF(e)$.
Therefore $LF(b)$ becomes:
\begin{eqnarray}
\begin{array}{lcl}
LF(b) &=& C(\Last_b) + Occ(\Last_b, b)\\
 	  &=& C(c) + Occ(c, b)\\
	  &=& C(c) + Occ(c, b_v - 1) + 1
\end{array}
\end{eqnarray}
...
Algorithm \ref{alg:fm-godownc} implements the operation \FunctionName{goDown} a symbol.
This algorithm computes two values of permutation $LF$ and thus runs in time $\Oh(1)$.
Conversely, \FunctionName{goDown} and \FunctionName{goRight} are provided by the generic algorithms \ref{alg:st-godown} and \ref{alg:st-goright} running in time $\Oh(\sigma)$.
...
By sampling one text position out of $\log^{1+\epsilon}{\Slen}$, for some $\epsilon > 0$, then $\Sarray^{\epsilon}$ consumes $\Oh(\frac{n{\Slen}}{\log^{\epsilon}{\Slen}})$ space and \FunctionCall{occurrences}{\StrieNode} returns all occurrences in $\Oh(o \cdot \log^{1+\epsilon}{\Slen})$ time \citep{Ferragina2000}.
...
Algorithm \ref{alg:st-exact-multi} matches simultaneously in $\StrieOf{\Texts}$ all patterns indexed in $\RtrieOf{\Patterns}$.
The traversal performed by algorithm \ref{alg:st-exact-multi} visits pairs of nodes in $\StrieOf{\Texts} \times \RtrieOf{\Patterns}$ whose entering edges have the same label.
Such traversal implicitly \emph{intersects} the two tries.
However, algorithm \ref{alg:st-exact-multi} is not symmetric: $\StrieOf{\Texts}$ and $\RtrieOf{\Patterns}$ cannot be interchanged.
The traversal stops whenever it reaches a leaf node in $\RtrieOf{\Patterns}$ and reports the occurrences pointed by all the leaves beneath the current node in $\StrieOf{\Texts}$.
