...
\begin{definition}
The \emph{suffix array} of a padded string $\String$ of length $\Slen$ is an array $\Sarray$ containing a permutation of the interval $\OInterval{\At{1}}{\Slen}$, \st $\Suffix{\String}{\SubscriptNV{\Sarray}{i}} <_{lex} \Suffix{\String}{\SubscriptNV{\Sarray}{i+1}}$ for all $1 \leq i < \Slen$.
\end{definition}
...
\begin{figure}[b]
\begin{center}
\caption[Example of (generalized) suffix array]{(Generalized) suffix array. (\subref{fig:sa}) Suffix array of the string {\ttfamily ANANAS\$}. (\subref{fig:gsa}) Generalized suffix array of the string collection $\Strings = \{$ {\ttfamily ANANAS$\$_{\At{1}}$}, {\ttfamily CACAO$\$_{\At{2}}$} $\}$.}
...
\midrule
\At{1} & (\At{1},\At{7}) & $\$_{\At{1}}$\\
\At{2} & (\At{2},\At{6}) & $\$_{\At{2}}$\\
\At{3} & (\At{2},\At{2}) & ACAO$\$_{\At{2}}$\\
\At{4} & (\At{1},\At{1}) & ANANAS$\$_{\At{1}}$\\
\At{5} & (\At{1},\At{3}) & ANAS$\$_{\At{1}}$\\
\At{6} & (\At{2},\At{4}) & AO$\$_{\At{2}}$\\
\At{7} & (\At{1},\At{5}) & AS$\$_{\At{1}}$\\
\At{8} & (\At{2},\At{1}) & CACAO$\$_{\At{2}}$\\
\At{9} & (\At{2},\At{3}) & CAO$\$_{\At{2}}$\\
\At{10} & (\At{1},\At{2}) & NANAS$\$_{\At{1}}$\\
\At{11} & (\At{1},\At{4}) & NAS$\$_{\At{1}}$\\
\At{12} & (\At{2},\At{5}) & O$\$_{\At{2}}$\\
\At{13} & (\At{1},\At{6}) & S$\$_{\At{1}}$\\
\end{tabular}
\end{center}
\end{subfigure}
...
The root node is represented by the interval $\OInterval{\At{1}}{\Slen}$ containing the whole suffix array.
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ \At{1}, \Slen, 0\}$;
...
\begin{eqnarray}
h(\Pattern) = 1 + \sum_{i=\At{1}}^{\AtEnd{q}}{\rho_0(\Subscript{\Pattern}{i}) \cdot \sigma^{q-i}}
\end{eqnarray}
...
The hash function $h$ allows to store in and retrieve from $D$ the left SA interval returned by algorithm~\ref{alg:sa-lower} for each $q$-gram, \ie $p \in \Sigma^q$, $\SubscriptNV{D}{h(\Pattern)} = \FunctionCall{L}{\At{1},\Slen,p}$.
...
\label{obs:lf-a}
\citep{Burrows1994}
For all $i \in \OInterval{\At{1}}{\Slen}$, the character $\Subscript{\Last}{i}$ precedes the character $\Subscript{\First}{i}$ in the original string $\String$.% (except when $\Subscript{\Last}{i} = \$$);
\end{observation}
\begin{observation}
\label{obs:lf-b}
\citep{Burrows1994}
