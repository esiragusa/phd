...
\If {\Call{isLeaf}{$\Tn$}}
	\State \Return \False
\EndIf
\State {$x.d \gets x.d+1$}
\State {$x.l \gets \Call{L}{x,c}$}
\State {$x.r \gets \Call{R}{x,c}$}
...
\EndIf
\State {$x.d \gets x.d+1$}
\State {$x.l \gets$ \Call{R}{$x,\epsilon$}}
%\While {$A[l] + d < n_{A[l]}$}
%	\State {$l \gets l+1$}
%\EndWhile
%\If {$l \geq r$}
%	\State \Return \False
%\EndIf
\State {$c_l \gets \Sc_{A[x.l] + x.d}$}
\State {$c_r \gets \Sc_{A[x.r] + x.d}$}
\If {$c_l \neq c_r$}
	\State {$x.r \gets$ \Call{R}{$x,c_l$}}
\EndIf
\State \Return $x.l < x.r$
\end{algorithmic}
\end{algorithm}
