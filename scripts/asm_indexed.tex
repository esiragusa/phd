...
\begin{definition}
The \emph{generalized suffix array} (GSA) of a padded string collection $\Strings$ (definition~\ref{def:coltd}), consisting of $\Scard$ strings of total length $\Slen$, is an array $\Sarray$ of length $\Slen$ containing a permutation of all pairs $(i,j)$ where $i$ points to a string $\AtCollection{\String}{i} \in \Strings$ and $j$ points to one of the $\AtCollection{\Slen}{i}$ suffixes of $\AtCollection{\String}{i}$.
...
The space consumption of the GSA is $\Slen \log{\Scard\Slen^*}$ bits, where $\Slen^* = \max{\,\AtCollection{\Slen}{i}}$.
...
\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\SubscriptNV{\Sarray}{\StrieNode.r} + \StrieNode.d = \AtCollection{\Slen}{\SubscriptNV{\Sarray}{\StrieNode.r}}$;
