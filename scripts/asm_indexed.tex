...
\While {$r_1 < r_2$}
	...
	\If {$\Strings_{A[i] + \Tn.d} \leq_{lex} c$}
		\State {$r_1 \gets i+1$}
	\Else
		\State {$r_2 \gets i$}
	\EndIf
...
\item \textsc{label}($\Tn$) returns $\Strings_{A[\Tn.l] + \Tn.d}$;
...
%	\State \Return \False
%\EndIf
\State {$c_l \gets \Strings_{A[\Tn.l] + \Tn.d}$}
\State {$c_r \gets \Strings_{A[\Tn.r] + \Tn.d}$}
\If {$c_l \neq c_r$}
...
Nonetheless, the top-down traversal needs to recover the substrings of $\Strings$ in \emph{forward} direction, as the suffix trie $\Si$ spells all forward substrings of $\Strings$.
