...
Before turning to succinct full-text indices, I briefly describe how I reused the text-indices just exposed to implement also tries and radix trees in the SeqAn library.
A trie is easily emulated by means of a partial SA.
I index only the first suffix of each string in the collection and subsequently construct the SA-based trie via quicksort in time $\Oh(n \log n)$, where $n$ is the cardinality of the string collection.
...
Some years after its introduction, \citeauthor{Ferragina2000} proposed the BWT as a method for full-text indexing.
