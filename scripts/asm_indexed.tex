...
I first consider the binary case $\Sigma = \mathbb{B} = \{ 0, 1 \}$ and subsequently the case of an arbitrary alphabet.
...
Finally, I briefly discuss how I implemented practical \emph{multi-level} RDs.
...
A binary \emph{two-levels} rank dictionary squeezes space consumption to $n+o(n)$ bits.
...
The RD supplements each block of $s$ with an occurrences summary for all symbols in $\Sigma$, using a matrix $R_{\sigma}$ of size $\frac{n}{b} \times \sigma$ entries.
...
Answering this query requires counting the number of occurrences of the character $c$ inside a block of length $b$.
...
This tree data structure hierarchically partitions the alphabet $\Sigma$ in balanced subsets, and accordingly decomposes the input string in \emph{subsequences} containing symbols from one subset.
The root of the WT represents all alphabet symbols, while each leaf represents exactly one symbol.
The WT encodes information inside each node using a separate binary RD.
Any query $\rank_c$ is decomposed as a sequence of binary rank queries, following the path from the root node to the leaf corresponding to symbol $c$.
The formal definition of WT follows.

\begin{definition}
\citep{Navarro2007}
The wavelet tree of a string $s \in \Sigma^*$ is a balanced binary tree of height $\lceil \log \sigma \rceil$, where the root node represents all symbols in $\Sigma$ and each leaf exactly one symbol.
Any non-leaf node $v$ represents the subset of symbols $\Sigma_v$ whose lexicographic rank is in range $[i,j]$ \ie $\Sigma_v = \{ c \in \Sigma : \rho(c) \in [i,j] \}$, its left child $u$ represents the subset $\Sigma_u$ of symbols in range $[i,\frac{i+j}{2}]$ while its right child $w$ represents in $\Sigma_w$ those in range $[\frac{i+j}{2}+1, j]$.
Node $v$ implicitly represents the subsequence $s^v$ of all symbols of $s$ in $\Sigma_v$ and explicitly encodes the binary decomposition of $s^v$ as a binary string $b^v$ \st $b^v_i = 0$ iff $s^v_i \in \Sigma_u$.
\end{definition}

\begin{figure}[h]
\begin{center}
\caption[Example of wavelet tree]{Example of wavelet tree of the DNA string $s=$ {\ttfamily CCCGCA}.}
\label{fig:wt_dna}
%\ttfamily
%\begin{tabular}{rcccc}
%$i$	& $s_{\frac{i}{2}}$	& $s_i$ & $R_{\sigma}[\frac{i}{b}]$ & $\sigma$\\
