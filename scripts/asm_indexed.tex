\chapter{Indexing methods}
\label{chr:index}

Suffix trees are elegant data structures but they are rarely used in practice.
Although suffix trees provide theoretically optimal construction and query time, their high space consumption prohibits practical indexing of large string collections.
...
One might be tempted to think that such memory requirements are not anymore a limiting factor as, at the time of this writing, even standard workstations come with 32~GB of main memory.
...
\While {$l_1 < l_2$}
	\State {$i \gets \left\lfloor \frac{l_1+l_2}{2} \right\rfloor$}
...
\While {$r_1 < r_2$}
	\State {$i \gets \left\lfloor \frac{r_1+r_2}{2} \right\rfloor$}
...
The former property is immediate and allows to recover the original string from its BWT alone, while the latter property makes the transformed string more amenable to compression \citep{Ferragina2005}.
...
Over the last years, the FM-index has been widely employed under different re-implementations by many popular bioinformatics tools, \eg Bowtie \citep{Langmead2009} and BWA \citep{Li2009}, and is now considered a fundamental method for the indexing of genomic sequences \citep{Vyverman2012}.
...
The BWT generalizes to string collections.
For a padded string collection $\Strings$ (definition~\ref{def:coltd}) and its cyclic shifts matrix sorted in lexicographical order, definition~\ref{def:bwt} still holds.
Note that there are different ways of treating this problem (see \citep{Mantaci2007}).
...
Function $Occ$, answering the question \emph{``how many times a given character $\StrieChar$ occurs in the prefix $\Prefix{\Last}{i}$?''}, has to be computed efficiently, ideally in constant time and succinct space.
...
The idea is to divide $\String$ into \emph{superblocks} of length $b^2$ and add another array $R^2$ to store the number of occurrences of the bit $1$ in $\String$ before each superblock.
...
\begin{tabular}{rcr}
%    &                          & \\
$i$	& $\Subscript{\String}{i}$ & $R_{\sigma}\left[\xfloor{i/2}][\text{A}, \, \text{C}, \, \text{G}, \, \text{T}\right]$\\
\midrule
$\At{1}$ & C & $\left[0, \,0, \,0, \,0\right]$\\
$\At{2}$ & T\\
\cline{2-3}
$\At{3}$ & \cell{s3}{C} & \cell{R3}{$\left[0, \, \cell{R3C}{1}, \, 0, \, 1\right]$}\\
\cell{i4}{$\At{4}$}   & \cell{s4}{G}\\
\cline{2-3}
$\At{5}$ & C & $\left[0, \,2, \,1, \,1\right]$\\
$\At{6}$ & A\\
\end{tabular}
%\around{i4}
%\eround{s3}{s4}
...
I finally give, for the first time to the best of my knowledge, algorithms solving multiple variants of indexing exact string matching and $k$-mismatches.
...
In the following, I assume the text $\Text$ to be indexing by its suffix trie $\StrieOf{\Text}$.
...
Algorithm \ref{alg:st-exact-multi} matches simultaneously in $\StrieOf{\Text}$ all patterns indexing in $\RtrieOf{\Patterns}$.
