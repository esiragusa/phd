...
Afterwards, I give generic string matching algorithms working on any of these data structures and at the same time provide experimental evaluations.
A \CC implementation of all algorithms and data structures is publicly available in source form within the SeqAn library \citep{Doering2008}.
...
Leaves pointing to the sorted suffixes are sufficient to perform exact string matching or even trie traversals.
...
Note that the trie can be iterated down spelling a full pattern within a single call of \textsc{L} and \textsc{R}: in line 6 of algorithms~\ref{alg:sa-lower}--\ref{alg:sa-upper}, instead of comparing a single character, it suffices to compare the full pattern to the current suffix.
...
%At this point, I am able to replace algorithm~\ref{alg:sa-exact} with algorithm~\ref{alg:qgram-exact}.
...
If the patterns are shorter or equal to the fixed length $q$,  the suffix array is accessed only to locate the occurrences, as the directory $D$ alone is sufficient to count the occurrences.
...
In the next subsections I give the fundamental ideas behind the BWT and the FM-index.
...
At the same time, I perform an experimental evaluation of each algorithm on various suffix trie implementations.
...
As text, I consider the C.~elegans reference genome (WormBase WS195), \ie a collection of 5 DNA strings of about 100~MB total length.
...
Algorithm \ref{alg:st-dfs} performs a top-down traversal in depth-first layout of a suffix trie.
...
I start by giving a simple algorithm performing exact string matching on a suffix trie.
...
I now give a backtracking algorithm solving $k$-mismatches on a generic suffix trie.
The method of backtracking a suffix tree has been first proposed in \citep{Ukkonen1993, Baeza1999}.
...
While traversing each branch of the trie, the algorithm incrementally computes the distance between the query and the spelled string.
...
I can compute $k$-differences on a suffix trie in two different ways. Algorithm~\ref{alg:st-edit-explicit} explicitly enumerates errors by recursing on the suffix trie. Algorithm~\ref{alg:st-edit} computes the edit distance on the suffix trie.
