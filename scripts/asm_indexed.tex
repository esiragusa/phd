...
I first consider the binary case $\Sigma_B = \{ 0, 1 \}$ and subsequently the case of an arbitrary alphabet.

\subsubsection{Binary alphabet}

I start by describing a simple \emph{one-level} rank dictionary answering rank queries in constant time but consuming $2n$ bits.
Later, I extend it as a \emph{two-levels} dictionary consuming only $n + o(n)$ bits.
...
\begin{figure}[h]
\begin{center}
\caption[Example of binary rank dictionaries]{Example of binary rank dictionaries for the string $s=$ {\ttfamily 010101100100}. On the one-level rank dictionary, $b=4$ and $\rank_1(s, 6) = R[2] + \rank_1(s_{5 \dots 8}, 2) = 3$. On the two-levels rank dictionary, $b=2$ and $\rank_1(s, 6) = R^2[2] + R[3] + \rank_1(s_{6 \dots 6}, 1) = 3$.}
\begin{tabular}{lcr}
...
A \emph{two-levels} rank dictionary squeezes space consumption to $n+o(n)$ bits.
...
I implemented generic \emph{multi-levels} rank dictionaries, where the block size $b$ is a template parameter adjustable at compile time.
...
\begin{figure}[h]
\begin{center}
\caption[Example of one-level DNA rank dictionary]{Example of one-level DNA rank dictionary for the string $s=$ {\ttfamily CCCGCA} under the binary encoding $\{ A,C,G,T \} = \{ 00,01,10,11 \}$ of the alphabet $\Sigma_{DNA}$.}
\label{fig:rd_dna}
\ttfamily
\begin{tabular}{rcccc}
...
Nonetheless, I want to recover substrings of $\Sc$ in \emph{forward direction}, as the suffix trie $\Si$ spells all forward substrings of $\Sc$.
...
\end{array}
\end{eqnarray}
Analogously, I rewrite $LF(e)$ as $C(c) + Occ(c, e_v)$.
Therefore, \textsc{goDown} a symbol computes two values of permutation $LF$ and runs in time $\Oh(1)$.
Conversely, \textsc{goDown} and \textsc{goRight} are implemented as generic algorithms \ref{alg:st-godown} and \ref{alg:st-goright}, thus run in time $\Oh(\sigma)$.
