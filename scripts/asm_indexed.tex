...
\begin{algorithm}[H]
\label{alg:fm-goroot}
\Algorithm{goRoot}{$\Tn \rightarrow \{ l, r \}$}
\begin{tabular}{lll}
\textbf{Input}  & $\Tn$ : pointer& FM-index node\\
\end{tabular}
\begin{algorithmic}[1]
\State {$l \gets 1$}
\State {$r \gets n$}
...
\Algorithm{goDown}{$\Tn \rightarrow \{ l, r, e \}, c$}
\begin{tabular}{lll}
\textbf{Input}  & $\Tn$ : pointer& FM-index node\\
				& $c$ : char& query character\\
\end{tabular}
\begin{algorithmic}[1]
\If {\Call{isLeaf}{$\Tn$}}
	\State \Return \False
...
If we sample one text position out of $\log^{1+\epsilon}{n}$, for some $\epsilon > 0$, then $A^{\epsilon}$ consumes $\Oh(\frac{n}{\log^{\epsilon}{n}})$ space and \textsc{getOccurrences}($\Tn$) returns all occurrences in $\Oh(o \cdot \log^{1+\epsilon}{n})$ time \citep{Ferragina2000}.
...
\Algorithm{Dfs}{$x,d$}
\begin{tabular}{lll}
\textbf{Input}  & $x$ : pointer& root node of the suffix trie\\
 			    & $d$ : integer& bounding depth\\
\end{tabular}
\begin{algorithmic}[1]
\If {$d > 0$}
	\If {\Call{goDown}{$x$}}
...
\begin{tabular}{lll}
\textbf{Input}  & $t$ : pointer& root node of the suffix trie of the text\\
 			    & $p$ : pointer& root node of the trie of the patterns\\
\textbf{Output} & list & all occurrences of any pattern in the text\\
\end{tabular}
\begin{algorithmic}[1]
\If {\Call{isLeaf}{$p$}}
