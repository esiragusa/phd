...
Consider an input string $s$ over $\Sigma_{\text{DNA}}$, of size $n$ bits and thus of length $n/\log{\sigma}$ symbols.
As in the binary case, this one-level RD partitions $s$ in blocks of $b$ symbols, now corresponding to $b \log{\sigma}$ bits.
The RD supplements each block of $s$ with a summary of occurrences for all symbols in $\Sigma$, using a matrix $R_{\sigma}$ of size $\xfloor{n/b} \times \sigma$ entries.
...
In order to answer this query in constant time, I consider blocks of $\xfloor{\log{n}/\log{\sigma}}$ symbols, \ie $b=\log{n}$ bits as in the binary RD case.
The matrix $R_{\sigma}$ has $\xfloor{n/\log{n}}$ entries, each one consuming $\sigma \log{n}$ bits.
