...
Figure \ref{fig:query-dna-exact-multi} shows that a simple lexicographical sort of the patterns (histogram Sorted) speeds up algorithm \ref{alg:st-exact-multi} on the SA and Esa by a factor of 2.
The same trick does not yield a significant speed-up on the $q$-gram index, as it is already cache local, nor on FM-indices.
Algorithm~\ref{alg:st-exact-multi} further reduces the traversal time.
Nonetheless, its runtime is dominated by the additional preprocessing time paid to construct the trie of the patterns.
%This algorithm turns out to be more useful within the multiple $k$-mismatches algorithm.
