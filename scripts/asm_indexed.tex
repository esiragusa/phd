...
The experimental evaluation compares algorithm~\ref{alg:st-exact-multi} (Dfs) with algorithm~\ref{alg:st-exact} processing patterns in random order (Naive) and in lexicographic order (Sorted).
Figure~\ref{fig:query-dna-exact-multi} shows the results.
In this evaluation, all patterns have length 15: runtimes shown in figure \ref{fig:query-dna-exact-multi} (histogram Naive) correspond to runtimes shown in figure \ref{fig:query-dna-exact} (plots at pattern length 15).
...
Algorithm~\ref{alg:st-exact-multi} (histogram Dfs) further reduces the traversal time.
...
The experimental evaluation compares algorithm~\ref{alg:st-hamming-multi} (Dfs) with algorithm~\ref{alg:st-hamming} processing patterns in random order (Naive) and in lexicographic order (Sorted).
In this evaluation, all patterns have length 30: runtimes shown in figure \ref{fig:query-dna-apx-multi} (histogram Naive) correspond to runtimes shown in figure \ref{fig:query-dna-apx} (plots at pattern length 30).
...
Algorithm \ref{alg:st-exact-multi} (histogram Dfs) reduces traversal time by a factor 5 on SA and Esa, thus the time to construct the trie of the patterns is easily justified.
