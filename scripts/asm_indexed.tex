...
We can implement \textsc{goDown} and \textsc{goRight} using only \textsc{R}.
...
\begin{algorithmic}[1]
\Procedure{goRoot}{$\Tn \rightarrow \{ l_h, r_h, l, r, d\}$}
	\State {$l_h \gets 1$}
...
\begin{algorithmic}[1]
\Procedure{goDown}{$\Tn \rightarrow \{ l, r, e \}$}
	\If {\Call{isLeaf}{$\Tn$}}
		\State \Return \False
	\EndIf
	\State {$e \gets \min_{lex}{\Sigma}$}
	\While {\Call{goDown}{$x,e$}}
		\State {$\Inc e$}
	\EndWhile
	\State \Return $l < r$
\EndProcedure
...
\begin{algorithmic}[1]
\Procedure{goRight}{$\Tn \rightarrow \{ l, r, e \}$}
	\If {\Call{isRoot}{$\Tn$}}
		\State \Return \False
	\EndIf
	\Repeat
		\State {$\Inc e$}
	\Until \Call{goDown}{$x,e$}
	\State \Return $l < r$
\EndProcedure
\end{algorithmic}
\end{minipage}
