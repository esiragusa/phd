...
This latter algorithm necessitates of a method capable of checking whether the edit distance at any node is within the imposed threshold $k$.
...
\If {$e = k$}
	\State \Call{ExactSearch}{$t,p$}
\Else 
	\State \Call{KDifferences}{$t,p_{2..|p|},e-1$}
	\ForAll {$\Cn \in \Ci(\Tn)$}
		\State \Call{KDifferences}{$t,p,e-1$}
		\If {$\Call{label}{\Cn} = p_1$}
			\State \Call{KDifferences}{$t,p_{2..|p|},e$}
		\Else
			\State \Call{KDifferences}{$t,p_{2..|p|},e-1$}
		\EndIf
	\EndFor
...
The traversal of this algorithm is the intersection between the labeled branches of the two tries.
