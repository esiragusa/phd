...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ 1, n, 0\}$;
...
In addition, I maintain the interval $[l_h,r_h]$ in $D$ and, in order to answer \FunctionCall{label}{\StrieNode}, the label $e$ of the edge entering the current node.
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ 1, n, 0, 1, \sigma^q, \epsilon \}$;
...
The wavelet tree of a string $s \in \Sigma^*$ is a balanced binary tree of height $\lceil \log \sigma \rceil$. The root represents all symbols in $\Sigma$ and each leaf exactly one symbol $\StrieChar \in \Sigma$.
Any non-leaf node $v$ represents some subset of symbols $\Sigma_v$ whose lexicographic rank is in range $\Interval[i,j]$ \ie $\Sigma_v = \{ c \in \Sigma : \rho(c) \in \Interval[i,j] \}$, its left child $\Last$ represents the subset $\Sigma_l$ of symbols in range $[i,\frac{i+j}{2}]$ while its right child $r$ represents in $\Sigma_r$ those in range $[\frac{i+j}{2}+1, j]$.
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ 1, n, \epsilon \}$;
%\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\StrieNode.r \in \mathbb{I}$;
\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\StrieNode.l_r = \$$;
\item \FunctionCall{label}{\StrieNode} returns $\StrieNode.e$.
\end{itemize}
