...
We start by giving the simple exact string matching algorithm using virtual suffix tree iterators.

\begin{algorithm}[h]
\caption{Exact string matching on a virtual suffix trie.}
\label{alg:vst-exact}
\begin{algorithmic}[1]
...
\begin{algorithm}[h]
\caption{Multiple exact string matching on a virtual suffix trie.}
...
\Procedure{KDifferences}{$t,p,D$}
	...
	\ElsIf {$\min{D} \leq k$}
		\If {\Call{goDown}{$t$}}
			\Repeat
				\State {$D' \gets$ \Call{DP}{$D, label(t), p$}}
