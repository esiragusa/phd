...
\begin{definition}
The \emph{suffix array} of a padded string $\String$ of length $\Slen$ is an array $\Sarray$ containing a permutation of the interval $\OInterval{\Ibase}{\Slen}$, \st $\Suffix{\String}{\SubscriptNV{\Sarray}{i}} <_{lex} \Suffix{\String}{\SubscriptNV{\Sarray}{i+1}}$ for all $1 \leq i < \Slen$.
\end{definition}
...
Pairs are ordered \st $\Suffix{\Strings}{\SubscriptNV{\Sarray}{i}} <_{lex} \Suffix{\Strings}{\SubscriptNV{\Sarray}{i+1}}$ for all $1 \leq i < \Slen$.
\end{definition}
...
When $\Slen < 2^{32}$, a 32 bit integer is sufficient to encode any value in the range $\OInterval{\Ibase}{\Slen}$.
...
\While {$l_1 < l_2$}
	...
	\If {$\Subscript{\Strings}{\SubscriptNV{\Sarray}{i} + \StrieNode.d} <_{lex} \StrieChar$}
		\State {$l_1 \gets i+1$}
	\Else
		\State {$l_2 \gets i$}
	\EndIf
...
\While {$r_1 < r_2$}
	...
	\If {$\Subscript{\Strings}{\SubscriptNV{\Sarray}{i} + \StrieNode.d} \leq_{lex} \StrieChar$}
		\State {$r_1 \gets i+1$}
	\Else
		\State {$r_2 \gets i$}
	\EndIf
...
Thus, while traversing the trie, I maintain the interval $\OInterval{l}{r}$ associated to the current node.
In addition, I also remember the depth $d$ of the current node.
The root node is represented by the interval $\OInterval{\Ibase}{\Slen}$ containing the whole suffix array.
The label of any edge entering some internal node at depth $d$ is defined by the $d$-th symbol within the corresponding suffix $\Suffix{\Strings}{\SubscriptNV{\Sarray}{i}}$ with $i \in \OInterval{l}{r}$.
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ \Ibase, \Slen, 0\}$;
%\item \FunctionCall{isRoot}{\StrieNode} returns true iff $d = 0$;
\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\SubscriptNV{\Sarray}{\StrieNode.r} + \StrieNode.d = \Slen_{\SubscriptNV{\Sarray}{\StrieNode.r}}$;
...
\State {$\StrieChar_r \gets \Subscript{\Strings}{\SubscriptNV{\Sarray}{\StrieNode.r} + \StrieNode.d}$}
\If {$\StrieChar_l \neq \StrieChar_r$}
...
%Note that line 6 of algorithms~\ref{alg:sa-lower}--\ref{alg:sa-upper} may involve a comparison beyond the end of strings in $\Strings$, hence I define $\Subscript{\Text}{i}$ as the empty word $\epsilon$ if $i > |\Text|$ and $\epsilon <_{lex} \StrieChar$ for all $\StrieChar \in \Sigma$.
...
With the aim of addressing $q$-grams in the directory $D$, I impose a canonical code on $q$-grams through a bijective function $h : \Sigma^q \rightarrow \OInterval{\Ibase}{\sigma^q}$ defined as in \citep{Knuth1973}:
\begin{eqnarray}
h(\Pattern) = 1 + \sum_{i=\Ibase}^{q}{\rho_0(\Subscript{\Pattern}{i}) \cdot \sigma^{q-i}}
\end{eqnarray}
where $p \in \Sigma^q$ is any $q$-gram and $\rho_0$ is the zero-based lexicographic rank defined on $\Sigma$ (recall the lexicographic rank function $\rho$ from definition~\ref{def:rho} and pose $\rho_0(\XSeq) = \rho(\XSeq) - 1$).
The canonical code assigned by $h$ preserves the lexicographical ordering for all words not longer than $q$, \ie $\XSeq <_{lex} \YSeq$ iff $h(\XSeq) < h(\YSeq)$ for all $\XSeq,\YSeq \in \Sigma^{\leq q}$.
The hash function $h$ allows to store in and retrieve from $D$ the left SA interval returned by algorithm~\ref{alg:sa-lower} for each $q$-gram, \ie $p \in \Sigma^q$, $\SubscriptNV{D}{h(\Pattern)} = \FunctionCall{L}{\Ibase,\Slen,p}$.
...
In addition, I maintain the interval $\OInterval{l_h}{r_h}$ in $D$ and, in order to answer \FunctionCall{label}{\StrieNode}, the label $e$ of the edge entering the current node.
...
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ \Ibase, \Slen, 0, \Ibase, \sigma^q, \epsilon \}$;
...
% (except when $\SubscriptNV{\Sarray}{i} = 1$).
...
\label{obs:lf-a}
\citep{Burrows1994}
For all $i \in \OInterval{\Ibase}{\Slen}$, the character $\Subscript{\Last}{i}$ precedes the character $\Subscript{\First}{i}$ in the original string $\String$.% (except when $\Subscript{\Last}{i} = \$$);
\end{observation}
\begin{observation}
\label{obs:lf-b}
\citep{Burrows1994}
...
\end{eqnarray}
where $C : \Sigma \rightarrow \OInterval{\Ibase}{\Slen}$ denotes the total number of occurrences in $\String$ of all characters alphabetically smaller than $\StrieChar$, and $Occ :  \Sigma \times \OInterval{\Ibase}{\Slen} \rightarrow \OInterval{\Ibase}{\Slen}$ the number of occurrences of character $\StrieChar$ in the prefix $\Prefix{\Last}{i}$.
...
The $j$-th entry of $R$ provides a summary of the number of occurrences of the bit $1$ in $\String$ before position $j b$, \ie $\SubscriptNV{R}{\Ibase} = 0$ and $\SubscriptNV{R}{j} = \rank_1(\String, j b - 1)$ for any $j > 1$.
...
\begin{figure}[t]
\begin{center}
\caption[Example of binary rank dictionaries]{Binary rank dictionaries (RDs) of the string $\String=$ {\ttfamily 010101100100}. (\subref{fig:rd1}) One-level RD with $b=4$; in the example, $\rank_1(\String, 6) = \SubscriptNV{R}{2} + \rank_1(\Suffix{\String}{5}, 2) = 3$. (\subref{fig:rd2}) Two-levels RD with $b=2$ (note that $R$ is now different from $b=4$); in the example, $\rank_1(\String, 6) = \SubscriptNV{R^2}{2} + \SubscriptNV{R}{3} + \rank_1(\Suffix{\String}{5}, 1) = 3$.}
...
\begin{eqnarray}
\rank_{\StrieChar}(\String,i) = \SubscriptNM{R_{\sigma}}{\xfloor{i/b}}{\rho(c)} + \rank_{\StrieChar}(\Suffix{\String}{\xfloor{i/b}}, \, i\imod{b}).
\end{eqnarray}
...
I represent the current node $\StrieNode$ by the elements $\{ l, r, e \}$, where $\OInterval{l}{r}$ represents the current suffix array interval and $e$ is the label of the edge entering the current node.
Therefore, I define the following node operations:
\begin{itemize}
\item \FunctionCall{goRoot}{\StrieNode} initializes $\StrieNode$ to $\{ \Ibase, \Slen, \epsilon \}$;
%\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\StrieNode.r \in \mathbb{I}$;
\item \FunctionCall{isLeaf}{\StrieNode} returns true iff $\StrieNode.\Subscript{\Last}{r} = \$$;
\item \FunctionCall{label}{\StrieNode} returns $\StrieNode.e$.
\end{itemize}
