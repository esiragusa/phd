...
Now, suppose the traversal is on an arbitrary node $v$ of known interval $[b_v, e_v]$ \st the path from the root to $v$ spells the substring $s_v$.
...
Within depth $10$, the TL FM-index is the fastest full-text index for traversals, while at depth 10 and 11 the $q$-gram index is faster.
...
The search time of forward indices becomes constant for patterns of length above 15, \ie when the index becomes sparse.
Conversely, the search time of backward (FM) indices is linear in the pattern length. 
As expected the SA alone is never competitive with the $q$-gram index.
In particular, the SA shows a runtime peak for patterns of length 10, due to the fact that binary search converges more slowly for shorter patterns.
In addition, the Esa is never faster than the $q$-gram index, despite its higher memory consumption.
Concerning FM-indices, the WT variant is almost twice as slow as the TL variant, due to the fact that it performs twice the number of random memory accesses.
Summing up, the TL FM-index is the fastest index to perform exact search of patterns within length 30, while the $q$-gram index is the fastest for patterns above length 30.
...
Figure \ref{fig:query-dna-apx} shows the results of the experimental evaluation of algorithm~\ref{alg:st-hamming}.
