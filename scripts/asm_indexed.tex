...
\begin{definition}
The \emph{suffix array} of a padded string $\String$ of length $\Slen$ is an array $\Sarray$ containing a permutation of the interval $\OInterval{\At{1}}{\Slen}$, \st $\Suffix{\String}{\SubscriptNV{\Sarray}{i}} \lexlt \Suffix{\String}{\SubscriptNV{\Sarray}{i+1}}$ for all $1 \leq i < \Slen$.
\end{definition}
...
Pairs are ordered \st $\Suffix{\Strings}{\SubscriptNV{\Sarray}{i}} \lexlt \Suffix{\Strings}{\SubscriptNV{\Sarray}{i+1}}$ for all $1 \leq i < \Slen$.
\end{definition}
...
%Note that line 6 of algorithms~\ref{alg:sa-lower}--\ref{alg:sa-upper} may involve a comparison beyond the end of strings in $\Strings$, hence I define $\Subscript{\Text}{i}$ as the empty word $\epsilon$ if $i > |\Text|$ and $\epsilon \lexlt \StrieChar$ for all $\StrieChar \in \Sigma$.
...
\begin{eqnarray}
h(\Pattern) = 1 + \sum_{i=\At{1}}^{\AtEnd{q}}{\lexrank{\Subscript{\Pattern}{i}} \cdot \sigma^{q-i}}
\end{eqnarray}
where $p \in \Sigma^q$ is any $q$-gram and $\lexrank{}$ is the lexicographic rank defined on $\Sigma$ (see definition~\ref{def:rho}). 
% pose $\lexrank_0{c} = \lexrank{c} - 1$.
...
\end{tabular}
\begin{algorithmic}[1]
\State {$\StrieNode.r_h \gets \StrieNode.r_h - \lexrank{c} \cdot \sigma^{\StrieNode.d}$}
...
Any non-leaf node $v$ represents some subset of symbols $\Sigma_v$ whose lexicographic rank is in range $\Interval{i}{j}$ \ie $\Sigma_v = \{ c \in \Sigma : \lexrank{c} \in \Interval{i}{j} \}$, its left child $\Last$ represents the subset $\Sigma_l$ of symbols in range $\Interval{i}{\frac{i+j}{2}}$ while its right child $r$ represents in $\Sigma_r$ those in range $\Interval{\frac{i+j}{2}+1}{j}$.
