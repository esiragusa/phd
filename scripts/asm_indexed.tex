...
\begin{definition}
The \emph{suffix array} of a padded string $\String$ of length $\Slen$ is an array $\Sarray$ containing a permutation of the interval $\Interval{1}{\Slen}$, \st $\Suffix{\String}{\Sarray[i]} <_{lex} \Suffix{\String}{\Sarray[i+1]}$ for all $1 \leq i < n$.
\end{definition}

\begin{definition}
The \emph{generalized suffix array} (GSA) of a padded string collection $\Strings$ (definition~\ref{def:coltd}), consisting of $\Scard$ strings of total length $\Slen$, is an array $\Sarray$ of length $\Slen$ containing a permutation of all pairs $(i,j)$ where $i$ points to a string $\String^i \in \Strings$ and $j$ points to one of the $\Slen_i$ suffixes of $\String^i$.
Pairs are ordered \st $\Suffix{\Strings}{\Sarray[i]} <_{lex} \Suffix{\Strings}{\Sarray[i+1]}$ for all $1 \leq i < n$.
\end{definition}
...
\While {$l_1 < l_2$}
	...
	\If {$\Strings_{\Sarray[i] + \StrieNode.d} <_{lex} \StrieChar$}
		\State {$l_1 \gets i+1$}
	\Else
		\State {$l_2 \gets i$}
	\EndIf
...
\While {$r_1 < r_2$}
	...
	\If {$\Strings_{\Sarray[i] + \StrieNode.d} \leq_{lex} \StrieChar$}
		\State {$r_1 \gets i+1$}
	\Else
		\State {$r_2 \gets i$}
	\EndIf
...
The label of any edge entering some internal node at depth $d$ is defined by the $d$-th symbol within the corresponding suffix $\Strings_{\Sarray[i]}$ with $i \in [l,r]$.
Any leaf node is defined (see section~\ref{sub:introindex}) to have all outgoing edges labeled by terminator symbols.
The occurrences below any node correspond by definition to the interval $[l,r]$ of $\Sarray$.
...
\item \textsc{isLeaf}($\StrieNode$) returns true iff $\Sarray[\StrieNode.r] + \StrieNode.d = n_{\Sarray[\StrieNode.r]}$;
\item \textsc{label}($\StrieNode$) returns $\Strings_{\Sarray[\StrieNode.l] + \StrieNode.d}$;
\item \textsc{occurrences}($\StrieNode$) returns $\Substring{\Sarray}{\StrieNode.l}{\StrieNode.r}$.
\end{itemize}
...
%\While {$\Sarray[l] + d < n_{\Sarray[l]}$}
%	\State {$l \gets l+1$}
%\EndWhile
%\If {$l \geq r$}
%	\State \Return \False
%\EndIf
\State {$\StrieChar_l \gets \Strings_{\Sarray[\StrieNode.l] + \StrieNode.d}$}
\State {$\StrieChar_r \gets \Strings_{\Sarray[\StrieNode.r] + \StrieNode.d}$}
\If {$\StrieChar_l \neq \StrieChar_r$}
...
Functions \textsc{L} (algorithm~\ref{alg:sa-lower}) and \textsc{R} (algorithm~\ref{alg:sa-upper}) compute in $\Oh(\log{\Slen})$ binary search steps the position in $\Sarray$ of the left and right intervals corresponding to the child node that is reached by going down the edge labeled by a given symbol $\StrieChar$.
...
\EndIf
\State {$\StrieNode.d \gets \StrieNode.d+1$}
\State {$\StrieNode.l \gets \Call{L}{\StrieNode,\StrieChar}$}
\State {$\StrieNode.r \gets \Call{R}{\StrieNode,\StrieChar}$}
\State \Return $\StrieNode.l < \StrieNode.r$
\end{algorithmic}
...
As they rely on a single call of \textsc{R}, their time complexity is $\Oh(\log{\Slen})$.
...
Nonetheless, the worst case runtime of algorithm~\ref{alg:st-exact} stays $\Oh(\Plen \log{\Tlen})$, as each step of the binary search requires a full lexicographical comparison between the pattern and any suffix of the text, which takes $\Oh(m)$ time in the worst case.
As shown in \citep{Manber1990}, the worst case runtime can be decreased to $\Oh(\Plen + \log{\Tlen})$ at the expense of additional $\Tlen \log{\Tlen}$ bits, by storing the precomputed longest common prefixes (LCP) between any two consecutive suffixes $\Suffix{\Strings}{\Sarray[i]}$, $\Suffix{\Strings}{\Sarray[i+1]}$ for all $1 \leq i < n$.
...
Algorithms \ref{alg:qgram-lower} and \ref{alg:qgram-upper} respectively show functions \textsc{L} and \textsc{R} using the directory $D$ instead of $\Sarray$.
...
I index only the first suffix of each string in the collection and subsequently construct the SA-based trie via quicksort in time $\Oh(n \log n)$, where $\Slen$ is the total length of the string collection.
...
In the following, consider the string $\String$ to be cyclic and its subscript $s_i$ to be \emph{modular}, \eg $s_0 = \String{\Slen}$ and $\String{n+i} = s_i$ for any $i \in \N$.
...
\begin{figure}[t]
\begin{center}
\caption[Example of Burrows-Wheeler transform]{Cyclic shifts matrix of the string {\ttfamily ANANAS\$}. Column $\String{\Sarray[i]-1}$ represents the Burrows-Wheeler transform.}
\label{fig:bwt}
\ttfamily
...
The BWT can be obtained in linear time by scanning the suffix array $\Sarray$ and assigning the symbol $\String{\Sarray[i]-1}$ to the $i$-th BWT symbol.
However, constructing the BWT from the SA is still not desirable, especially for small alphabets, as the SA consumes $n \log{\Slen}$ bits in addition to the $n \log{\sigma}$ bits of the BWT.
...
For convenience, I denote the first column $\String{\Sarray[i]}$ by $f$ and the last column $\String{\Sarray[i] - 1}$ by $l$.
...
The full string $\String$ is recovered by starting in $f$ at the position of \$ and following the cycle defined by the permutation $\Psi$.
...
\label{obs:lf-a}
\citep{Burrows1994}
For all $i \in [1,n]$, the character $l_i$ precedes the character $f_i$ in the original string $\String$.% (except when $l_i = \$$);
\end{observation}
\begin{observation}
\label{obs:lf-b}
\citep{Burrows1994}
For all characters $c \in \Sigma$, the $i$-th occurrence of $\StrieChar$ in $f$ corresponds to the $i$-th occurrence of $\StrieChar$ in $l$.
\end{observation}

These observation are evident, indeed $f = \String{\Sarray[i]}$ and $l = \String{\Sarray[i] - 1}$ (see figure~\ref{fig:lf_psi}).
Given the two above observations, \cite{Ferragina2000} define the permutation $LF$ as:
\begin{eqnarray}
LF(i) = C(l_i) + Occ(l_i, i)
\end{eqnarray}
where $C : \Sigma \rightarrow \Interval{1}{n}$ denotes the total number of occurrences in $\String$ of all characters alphabetically smaller than $\StrieChar$, and $Occ :  \Sigma \times \Interval{1}{n} \rightarrow \Interval{1}{n}$ the number of occurrences of character $\StrieChar$ in the prefix $\Prefix{l}{i}$.
...
The question \emph{``how many times a given character $c$ occurs in the prefix $\Prefix{l}{i}$?''}, has to be answered efficiently, ideally in constant time and linear space.
...
\begin{definition}
Given a string $\String$ over an alphabet $\Sigma$ and a character $c \in \Sigma$, $\rank_c(s, i)$ returns the number of occurrences of $c$ in the prefix $\Prefix{\String}{i}$.
\end{definition}
...
By doing so, \cite{Jacobson1989} shows how to answer rank queries in constant time (on the RAM model) using $n+o(n)$ bits for an input binary string of $\Slen$ bits.
...
The $j$-th entry of $R$ provides a summary of the number of occurrences of the bit $1$ in $\String$ before position $j b$, \ie $R[1] = 0$ and $R[j] = \rank_1(s, j b - 1)$ for any $j > 1$.
...
Consider an input string $\String$ of length $\Slen$ over $\Sigma$, thus consisting of $n \log{\sigma}$ bits.
As in the binary case, this one-level RD partitions $\String$ in blocks of $b$ symbols.
It complements the string $\String$ with a matrix $R_{\sigma}$ of size $\xfloor{n/b} \times \sigma$ entries, summarizing the number of occurrences for each symbol in $\Sigma$.
...
Node $v$ implicitly represents the subsequence $\String^v$ of all symbols of $\String$ in $\Sigma_v$ and explicitly encodes its decomposition as a binary string $b^v$ \st $b^v_i = 0$ if $\String^v_i \in \Sigma_l$ and 1 otherwise.
\end{definition}
...
The rank of symbol $c$ in $\String^v$ is established as $\rank_0(b^v, j)$, where $j$ is the rank of $c$ in the parent node or $i$ in the root node.
...
At the same time, the WT does not need to store the original input string $\String$ of $n \log \sigma$ bits.
...
\item \textsc{label}($\StrieNode$) returns $\StrieNode.e$.
\end{itemize}
%In order to answer \textsc{isLeaf}, I consider the set $\mathbb{I}$ of positions of the terminator symbols $\String^i$ in the BWT.
...
All these characters $c$ are in $l_{b_v \dots e_v}$, since $l_i$ is the character $\Strings_{\Sarray[i] - 1}$ preceding the suffix pointed by $\Sarray[i]$.
...
\If {\Call{isLeaf}{$\StrieNode$}}
	\State \Return \False
\EndIf
\State {$x.l \gets \Call{LF}{x.l,\StrieChar}$}
\State {$x.r \gets \Call{LF}{x.r,\StrieChar}$}
\State {$x.e \gets c$}
...
By sampling one text position out of $\log^{1+\epsilon}{\Slen}$, for some $\epsilon > 0$, then $\Sarray^{\epsilon}$ consumes $\Oh(\frac{n{\Slen}}{\log^{\epsilon}{\Slen}})$ space and \textsc{occurrences}($\StrieNode$) returns all occurrences in $\Oh(o \cdot \log^{1+\epsilon}{\Slen})$ time \citep{Ferragina2000}.
...
%Algorithm~\ref{alg:st-exact} is correct since each path from the root to any internal node of the suffix trie $\StrieOf{\Strings}$ spells a different unique substring of $\Text$; consequently all equal substrings of $\Text$ are represented by a single common path.
If \textsc{goDown} is implemented in constant time and \textsc{occurrences} in linear time, all occurrences of $\Pattern$ into $\Text$ are found in optimal time $\Oh(\Plen+o)$, where $\Plen$ is the length of $\Pattern$ and $o$ its number of occurrences in $\Text$.
