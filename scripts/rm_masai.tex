...
Finally, I chose to parameterize our filter by the seed length rather than by the number of seeds.
...
Below I discuss the performance of these indices in our specific application, while I refer the reader to chapter~\ref{chr:index} for their extensive explanation.

\subsubsection{Indexing the reference genome}

I initially chose the ESA over LST because of better construction times.
Indeed, I dispose of a linear time construction algorithm for the ESA (an adaptation of the DC7 algorithm \citep{Dementiev2008} to multiple sequences \citep{Weese2013} for the generalized SA, followed by the algorithms proposed in \citep{Kasai2001,Abouelhoda2004}), while our LST construction algorithm takes quadratic time (using the radix sort based \emph{wotd}-algorithm \citep{Giegerich1999}).
...
However, with surprise I found out that, within our application, the SA had equal or better performance than the ESA (see figure~\ref{?}).
...
Therefore, to cut the most expensive binary searches, I put a $q$-gram index on top of the SA and extended our generic suffix trie top-down traversal accordingly (see section~\ref{sec:index:qgram}).
...
In definitive, I prefer the SA as it provides a good compromise between query speed and memory consumption.
...
As our multiple search algorithm requires a trie of the seeds, I also engineered an efficient trie implementation.
...
I index only the first suffix of each seed in the collection and construct the SA-based trie via quicksort in time $\Oh(n \log n)$, where $n$ is the number of seeds; then, our top-down traversal based on binary search still works.
...
As the memory footprint of the trie is negligible within our application, I chose the LST to perform multiple backtracking of approximate seeds.
...
RazerS~3~\citep{RazerS3} already used Myers' algorithm.
...
In section \label{sup:masai:param}, I give the exact parameterization for the read mappers considered in our evaluation.
...
Similarly to \citep{Bowtie2}, I used the read simulator Mason \citep{SeqAnReadSimulator} with default profile settings to simulate from each whole genome 100\,k reads of length 100\,bp having sequencing errors distributed like in a typical Illumina run.
...
For a more fair and thorough comparison, I also consider BWA and Bowtie\,2 as all-mappers (Soap\,2 cannot be configured accordingly).
...
Similarly to \citep{Shrimp2}, this experiment considers simulated reads containing sequencing errors, SNPs and indels, such that each read has an edit distance of at most 5 to its genomic origin.
