...
\begin{figure}[h]
\begin{center}
\caption{Dotplot of the strings $x=AAAA$ and $y=CCCC$. The highlighted trace corresponds to the alignment of Example~\ref{ex:alignment}.}
\label{fig:dotplot}
...
Given two strings $x,y$, for all $1 \leq i \leq n$ and $1 \leq j \leq m$ we define with $d(x_{1..i},y_{1..j})$ the edit distance between their prefixes $x_{1..i}$ and $y_{1..j}$.
...
The recurrence relation can be computed in time $\Oh(nm)$ using a table of $(n+1) \times (m+1)$ cells. 
The table can be filled in four different ways: column-wise, row-wise, diagonal-wise or antidiagonal-wise.
An optimal alignment can be computed via traceback in time $\Oh(n + m)$.
However, only $\Oh(\min\{ n, m \})$ space is required to compute the the sole distance without any alignment.
...
\begin{eqnarray}
d_E(\epsilon, y_{1..j}) = 0 \text{ for all } 1 \leq j \leq m
\end{eqnarray}
...
\begin{eqnarray}
d_E(x_{1..m},y_{1..j}) \leq k \text{ for all } 1 \leq j \leq m.
\end{eqnarray}
