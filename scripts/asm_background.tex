...
\begin{definition}
The \emph{trie} $\StrieOf{\Strings}$ of $\Strings$ is a lexicographically ordered tree data structure having one node designated as the root and $c$ leaves, denoted as $\Ln_1 \dots \Ln_c$, where leaf $\Ln_i$ points to string $s^i$.
...
The \emph{suffix trie} $\StrieOf{\Strings}$ has $n$ leaves, where leaf $\Ln_i$ points to suffix $s_{i..n}$.
\end{definition}
...
Given a trie $\StrieOf{\Strings}$, I define the following operations inspecting any pointed node $\Tn$ of $\StrieOf{\Strings}$:
\begin{itemize}
