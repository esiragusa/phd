...
I denote by $\Substring{\String}{i}{j}$ the \emph{concatenated} symbols $s_i \dots s_j$, and by $|s|=n$ the length of $\String$.
...
A string $\XSeq$ is a \emph{prefix} of another string $\YSeq$ iff $x = \Prefix{\YSeq}{i}$ for some $1 \leq i \leq |\YSeq|$, a \emph{suffix} iff $x = \Suffix{\YSeq}{i}$ for some $1 \leq i \leq |\YSeq|$, 
and a \emph{substring} iff $x = \Substring{\YSeq}{i}{j}$ for some $1 \leq i \leq j \leq |\YSeq|$.
...
A string $\XSeq$ occurs in a string $\YSeq$ iff $\XSeq$ is a substring of $\YSeq$.
Given $x = \Substring{\YSeq}{i}{j}$, the occurrence of $\XSeq$ starts at position $i$ and ends at position $j$ in $\YSeq$.
...
\begin{definition}
\label{def:rho}
Given an alphabet $\Sigma$ of size $\sigma$, I denote the \emph{lexicographic rank} of any alphabet symbol by the function $\rho : \Sigma \rightarrow \Interval{1}{\sigma}$, \st $\rho(a) < \rho(b) \iff a < b$ for any distinct $a,b \in \Sigma$.
\end{definition}
The \emph{lexicographical order} $<_{lex}$ between two non-empty strings $\XSeq,\YSeq$ is defined as $\XSeq <_{lex} \YSeq \iff x_1 < y_1, \text{ or } x_1 = y_1 \text{ and } \Suffix{\XSeq}{2} <_{lex} \Suffix{\YSeq}{2}$.

\emph{Generalized} full-text indices work on string collections.
A \emph{string collection} is an ordered multiset $\Strings = \{ \String^1, \String^2, \dots, \String^c \}$ of non necessarily distinct strings over a common alphabet $\Sigma$.
I denote by $\norm{\Strings}=\sum_{i=1}^{c}{|\String^i|}$ the total length of the string collection.
I extend the above definitions of prefix, suffix and substring also to multisets, \eg $\Strings_{(d,i) \dots (d,j)}$ denotes the substring $\Substring{\String^d}{i}{j}$.
...
\begin{definition}
\label{def:coltd}
Given a string collection $\Strings$ over $\Sigma$, I call \emph{padded string collection} the collection consisting of strings $\String^i \in \Strings$ padded with terminator symbols $\$^i$ \st $\rho(\$^i) < \rho(\$^j) \iff i < j$.
\end{definition}
...
Given two strings $\XSeq,\YSeq$ of equal length $\Xlen$, the string $\XSeq$ is easily transformed into the string $\YSeq$ by substituting (or replacing) all symbols $x_i$ \st $x_i \neq y_i$ into $y_i$, for $1 \leq i \leq \Xlen$.
However, if the two strings have different lengths, some symbols from $\XSeq$ must be necessarily inserted or deleted in order to obtain $\YSeq$.
...
d_E(\Prefix{\XSeq}{i-1},\Prefix{\YSeq}{j-1})&+&\delta(x_i, y_j)
\end{array}
...
\For {$i \gets 1 \, \To \Xlen$}
	...
	\For {$j \gets 1 \, \To \Ylen$}
		\State {$D[i,j] \gets \min \, \{ \, D[i-1,j] + 1, D[i,j-1] + 1, D[i-1,j-1] + \delta(x_i, y_j) \, \}$}
	\EndFor
\EndFor
\State \Return D[\Ylen,\Xlen]
\end{algorithmic}
\label{alg:dp-edit}
\end{algorithm}
\end{minipage}
...
Algorithm \ref{alg:dp-edit} computes the above recurrence relation in time $\Oh(\Xlen\Ylen)$ using a dynamic programming table $D$ of $(\Xlen+1) \times (\Ylen+1)$ cells, where cell $D[i,j]$ stores the value of $d_E(\Prefix{\XSeq}{i},\Prefix{\YSeq}{j})$.
...
\begin{definition}
The \emph{trie} $\StrieOf{\Strings}$ of $\Strings$ is a lexicographically ordered tree data structure having one node designated as the root and $\Scard$ leaves, denoted as $\StrieLeaf{1}, \dots, \StrieLeaf{\Scard}$, where leaf $\StrieLeaf{i}$ points to string $\String^i$.
...
\begin{definition}
Given a padded string $\String$ (definition~\ref{def:strd}) of length $\Slen$, the suffix trie $\StrieOf{\Strings}$ is the trie of all suffixes of $\String$.
The \emph{suffix trie} $\StrieOf{\Strings}$ has $\Slen$ leaves, where leaf $\StrieLeaf{i}$ points to suffix $\Suffix{\String}{i}$.
\end{definition}
%The suffix trie generalizes to index string collections.
\begin{definition}
Given a padded string collection $\Strings$ (definition~\ref{def:coltd}), any leaf of the \emph{generalized suffix trie} is labeled with a pair $(i,j)$ where $i$ points to the string $\String^i \in \Strings$ and $1 \leq j \leq n_i$ points to one of the $n_i$ suffixes of $\String^i$.
...
\If {\Call{isLeaf}{$\StrieNode$}}
	\State \Return \False
\EndIf
\If {\Call{goDown}{$\StrieNode, \min_{lex}{\Sigma}$}}
	\State \Return \True
\Else
...
\If {\NotB \Call{isRoot}{$\StrieNode$}}
	\While {$c \gets \text{next}_{lex}{\Call{label}{\StrieNode}}$}
		\State {$\Call{goUp}{\XSeq}$}
		\If {$\Call{goDown}{x, c}$}
			\State \Return \True
		\EndIf
	\EndWhile
