...
Given the binary sequence $s \in \Sigma_B$, we partition it in blocks of $\log{n}$ bits.
We attach to the binary sequence $s$ an array $R$ of length $\log{n}$, where the $i$-th entry gives a summary of the number of occurrences of the bit $1$ in $s_{1 \dots i \log{n}}$.
At this point we are already able to answer $rank_1(s, i)$ in constant time by 
\begin{inparaenum}[(i)]
\item \label{itm:fetch} fetching the summary from $R[\frac{i}{\log{n}}]$ and
\item \label{itm:count} adding the number of occurrences of the bit 1 inside the subsequence $s_{\frac{i}{\log{n}} \dots i}$,
\end{inparaenum}
as we can always perform operation~\ref{itm:fetch} in constant time and operation~\ref{itm:count} in constant time on modern processors \citep{Intel}.
Note that $rank_0(s, i) = i - rank_1(s, i)$.

The array $R$ stores $\frac{n}{\log{n}}$ positions and each position in $s$ requires $\log{n}$ bits, so we are taking $n$ bits of space for $R$.
...
Thus, this time we use a matrix $R$ of $\sigma \times b$ entries, each one taking $\log{n}$ space.
Again, we answer $rank_c(s, i)$ in constant time by
\begin{inparaenum}[(i)]
\item \label{itm:fetch} fetching the summary from $R[rank(c)][\frac{i}{b}]$ and
\item \label{itm:count} adding the number of occurrences of the bit 1 inside the subsequence $s_{\frac{i}{b} \dots i}$,
\end{inparaenum}
