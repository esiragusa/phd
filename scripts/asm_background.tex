...
Note how the matrix $M$ is related to the suffix array $A$ of $t$: the cyclic rotation in the $i$-th row is $M[i:] = t_{A[i] \dots n} t_{1 \dots A[i-1]}$ (except when $A[i] = 1$).

\begin{definition}
The BWT of $t$ is the string $l$ obtained concatenating the symbols in the last column of $M$, \ie $l = M[:n]$.
\end{definition}
...
Various direct BWT construction algorithms have been recently proposed \citep{Bauer2013, Crochemore2013}, as constructing the suffix array is not appealing due to its space consumption of $n \log{n}$ bits.

%\subsubsection{Inversion}

We now describe how to reverse the BWT to reconstruct the original text.
...
Given the above observations, we can define two permutations $LF : [1,n] \rightarrow [1,n]$ and $\Psi : [1,n] \rightarrow [1,n]$, where the value of $LF(i)$ gives the position $j$ in $f$ where character $l_i$ occurs and the value $\Psi(j)$ gives back the position $i$ in $l$ where $f_j$ occurs.
