...
An \emph{alphabet} $\Sigma= \{ a_{\FirstI}, a_{\NextI}, \dots , a_{\LastI{\sigma}} \}$ is a finite ordered set of symbols (or characters) $a_i$.
A \emph{string} (or word) $\String=\Subscript{\String}{\FirstI} \Subscript{\String}{\NextI} \dots \Subscript{\String}{\LastI{\Slen}}$ over an alphabet $\Sigma$ is a finite sequence of symbols $\Subscript{\String}{i} \in \Sigma$.
...
\begin{definition}
\label{def:rho}
Given an alphabet $\Sigma$ of size $\sigma$, I denote the \emph{lexicographic rank} of any alphabet symbol by the function $\rho : \Sigma \rightarrow \OInterval{\Ibase}{\sigma}$, \st $\rho(a) < \rho(b) \iff a < b$ for any distinct $a,b \in \Sigma$.
\end{definition}
The \emph{lexicographical order} $<_{lex}$ between two non-empty strings $\XSeq,\YSeq$ is defined as $\XSeq <_{lex} \YSeq \iff \Subscript{\XSeq}{\FirstI} < \Subscript{\YSeq}{\FirstJ}, \text{ or } \Subscript{\XSeq}{\FirstI} = \Subscript{\YSeq}{\FirstJ} \text{ and } \Suffix{\XSeq}{\NextI} <_{lex} \Suffix{\YSeq}{\NextJ}$.

\emph{Generalized} full-text indices work on string collections.
A \emph{string collection} is an ordered multiset $\Strings = \{ \String^{\FirstI}, \String^{\NextI}, \dots, \String^{\LastI{c}} \}$ of non necessarily distinct strings over a common alphabet $\Sigma$.
I denote by $\norm{\Strings}=\sum_{i=\FirstI}^{\LastI{c}}{|\String^i|}$ the total length of the string collection.
...
An \emph{array} (or table, or vector) $V=\Subscript{V}{\FirstI} \Subscript{V}{\NextI} \dots \Subscript{V}{\LastI{\Slen}}$ is a finite sequence of numbers $\Subscript{V}{i} \in \N_0$ (or any subset of $\N_0$).
...
$$X=
\begin{pmatrix}
  \SubscriptM{X}{\FirstI}{\FirstJ} & \SubscriptM{X}{\SameI}{\NextJ} & \cdots & \SubscriptM{X}{\SameI}{\LastJ{n}} \\
  \SubscriptM{X}{\NextI}{\FirstJ} & \SubscriptM{X}{1}{\SameI} & \cdots & \SubscriptM{X}{\SameI}{\LastJ{n}} \\
  \vdots & \vdots  & \ddots & \vdots  \\
  \SubscriptM{X}{\LastI{m}}{\FirstJ} & \SubscriptM{X}{\LastI{m}}{\NextJ} & \cdots & \SubscriptM{X}{\LastJ{m}}{\LastJ{n}}
\end{pmatrix}$$
