...
Given a collection of padded strings $\Sc = \{ s^1, s^2, \dots, s^c \}$, any leaf of the \emph{generalized suffix trie} is labeled with a pair $(i,j)$ where $i$ points to the string $s^i \in \Sc$ and $1 \leq j \leq n$ points to one of the $n$ suffixes of $s^i$.
Thus each path from the root to a leaf $(i,j)$ spells the suffix $s^i_{j..n}$.
...
%Assume \wlogs a string $s$ of length $n$, padded with a \emph{terminator symbol} $\$$ not being part of the string alphabet $\Sigma$\footnote{The terminator symbol is necessary to ensure that no suffix $s_{i..n}$ is a prefix of another suffix $s_{j..n}$.}.
...
Given a trie $\Ti$, we denote by $i$ the \emph{iterator} pointing to an arbitrary node $\Tn$ of $\Ti$.
On the iterator $i$, we define the following operations inspecting the pointed node:
\begin{itemize}
%\item \textsc{isRoot}($i$)
%\item \textsc{isLeaf}($i$)
%\item \textsc{depth}($i$)
\item \textsc{label}($i$) returns the symbol labeling the edge entering the node $\Tn$;
\item \textsc{getOccurrences}($i$) returns the list of positions pointed by leaves below node $\Tn$;
\end{itemize}
and the following operations moving the pointed node and returning $true$ on success:
\begin{itemize}
\item \textsc{goDown}($i$) moves $i$ to the lexicographically smallest child of $\Tn$;
\item \textsc{goDown}($i, c$) moves $i$ to the child of $\Tn$ whose entering edge is labeled by $c$;
\item \textsc{goRight}($i$) moves $i$ to the lexicographically next child of $\Tn$;
\item \textsc{goUp}($i$) moves $i$ to the parent node of $\Tn$.
\end{itemize}
...
If the search ends up in a node $\Tn$, each leaf $\Ln_i$ below $\Tn$ points to a distinct suffix $t_{i..n}$ such that $t_{i..i+m} = p$.
