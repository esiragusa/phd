...
\begin{itemize}
\item \FunctionCall{goNext}{s} moves to $\Subscript{\String}{i+1}$;
\item \FunctionCall{goPrevious}{s} moves to $\Subscript{\String}{i-1}$;
...
Indeed, the DP matrix $D$ needs not to be filled entirely, as the cells outside a \emph{band} of width $k+1$ around the $0$-diagonal are not \emph{coaccessible} from the \emph{sink} cell $\SubscriptNM{D}{\Ylen}{\Xlen}$.
