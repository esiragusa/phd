...
%Finally, any subset of $\Sigma^*$ is called a language over $\Sigma$.
\end{definition}
...
Because some symbols in one string are not related to any symbol in the other string, \ie some symbols are inserted or removed, a gap symbol $-$, not being part of the string alphabet $\Sigma$, has to be introduced first.
...
\end{eqnarray}
Consequently, the distance function $d : \Sigma^{*} \times \Sigma^{*} \rightarrow \R_0^{+}$ is defined to take the minimum cost over all possible alignments of $x,y$:
\begin{eqnarray}
...
It is obtained by defining $\omega(\alpha,\beta)$ as in the edit distance, and by setting all $\omega(\alpha,-)$ and $\omega(-,\beta)$ to be $\infty$ in order to disallow indels.
\end{definition}

\begin{definition}
\label{def:edit}
The \emph{Levenshtein} or \emph{edit distance} between two strings $x,y \in \Sigma^{*}$ is defined as the function $d_E : \Sigma^{*} \times \Sigma^{*} \rightarrow \N_0$ counting the \emph{minimum} number of edit operation necessary to transform $x$ into $y$.
...
Given two strings $x,y$ of length $m,n$, for all $1 \leq i \leq m$ and $1 \leq j \leq n$ I define with $d(x_{1..i},y_{1..j})$ the distance between their prefixes $x_{1..i}$ and $y_{1..j}$.
...
The sole distance without any alignment is computed in space $\Oh(\min\{ n, m \})$, as only column $D[:j-1]$ is needed to compute column $D[:j]$ (or row $D[i-1:]$ to compute $D[i:]$), and the table $D$ can be filled either column-wise or row-wise. %\footnote{Note that $D$ can be filled also diagonal-wise or antidiagonal-wise.}.
...
I now define \emph{exact string matching}, one of the most fundamental problem in stringology.
\begin{definition}
\citep{Gusfield1997}
...
For $k$-mismatches and $k$-differences, it must hold $k > 0$ as the case $k = 0$ corresponds to exact string matching, and $k < m$ as a pattern occurs at any position in the text if all its $m$ characters are substituted.
\begin{definition}
...
Among its virtues \citep{Apostolico1985}, the suffix tree natively provides exact string matching in optimal time and also approximate string matching via backtracking \citep{Ukkonen1993}.
...
Online filtering methods try to jump over the text while scanning it, while offline filtering methods use an index to place anchors in the text.
...
Note that the suffix trie so defined contains $\Oh(n^2)$ nodes, yet I only consider the suffix trie as an idealized data structure to expose generic algorithms.
...
For this reason, in the following of this manuscript I always consider \wlogs suffix tries instead of suffix trees.
...
I now give a set of generic operations to traverse tries in a top-down fashion.
...
Filtering methods work in two stages: they first discard uninteresting portions of the text and subsequently use a conventional method to verify the remaining portions.
These methods yield practical algorithms with good \emph{average} running times.
Whenever filtration is effective, the conventional method is applied to a relatively small portion of the text.
In this case, the additional filtration stage pays off.
Conversely, under some extreme circumstances, it pays off to directly apply the conventional method to the whole text.
...
Under the uniform Bernoulli model, where each symbol of $\Sigma$ occurs with probability $\frac{1}{\sigma}$, \citet{Navarro2000} estimate that $\epsilon < 1 - \frac{1}{\sigma}$ is a conservative bound on the error rate which ensures few matches, and for which filtering algorithms are effective.
For higher error rates, non-filtering methods, either online or offline, work better.

Filtering methods apply combinatorial criteria to determine which portions of the text are relevant and must be retained.
...
The combinatorial criterion underlying a filter is of paramount importance as it provides guarantees on filtration efficiency, both in terms of sensitivity and specificity.
...
Practical applications, \eg read mapping, often do not require strictly lossless filtration.
...
In this manuscript, I consider two classes of combinatorial filtering methods: those based on \emph{seeds} and those based on \emph{$q$-grams}.
The first class of filters partition the pattern into \emph{non-overlapping} factors called seeds.
...
Simple lemmata give lower bounds on the number of $q$-grams that must be present in a narrow window of the text, as necessary condition for an approximate occurrence of the pattern.
