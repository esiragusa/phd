...
An \emph{alphabet} $\Sigma= \{ a_0, a_1, \dots , a_{\sigma-1} \}$ is a finite ordered set of symbols (or characters) $a_i$.
A \emph{string} (or word) $\String=\Subscript{\String}{\Ibase} \Subscript{\String}{1} \dots \Subscript{\String}{\Slen-1}$ over an alphabet $\Sigma$ is a finite sequence of symbols $\Subscript{\String}{i} \in \Sigma$.
I denote by $\Substring{\String}{i}{j}$ the \emph{concatenated} symbols $\Subscript{\String}{i} \dots \Subscript{\String}{j}$, and by $|\String|=\Slen$ the length of $\String$.
...
A string $\XSeq$ is a \emph{prefix} of another string $\YSeq$ iff $\XSeq = \Prefix{\YSeq}{i}$ for some $1 \leq i \leq |\YSeq|$, a \emph{suffix} iff $\XSeq = \Suffix{\YSeq}{i}$ for some $1 \leq i \leq |\YSeq|$, 
and a \emph{substring} iff $\XSeq = \Substring{\YSeq}{i}{j}$ for some $1 \leq i \leq j \leq |\YSeq|$.
...
Given $\XSeq = \Substring{\YSeq}{i}{j}$, the occurrence of $\XSeq$ starts at position $i$ and ends at position $j$ in $\YSeq$.
...
\begin{definition}
\label{def:rho}
Given an alphabet $\Sigma$ of size $\sigma$, I denote the \emph{lexicographic rank} of any alphabet symbol by the function $\rho : \Sigma \rightarrow \Interval{\Ibase}{\sigma}$, \st $\rho(a) < \rho(b) \iff a < b$ for any distinct $a,b \in \Sigma$.
\end{definition}
The \emph{lexicographical order} $<_{lex}$ between two non-empty strings $\XSeq,\YSeq$ is defined as $\XSeq <_{lex} \YSeq \iff \Subscript{\XSeq}{\Ibase} < \Subscript{\YSeq}{\Ibase}, \text{ or } \Subscript{\XSeq}{\Ibase} = \Subscript{\YSeq}{\Ibase} \text{ and } \Suffix{\XSeq}{1} <_{lex} \Suffix{\YSeq}{1}$.
...
An \emph{array} (or table, or vector) $V=\Subscript{V}{\Ibase} \Subscript{V}{1} \dots \Subscript{V}{\Slen-1}$ is a finite sequence of numbers $\Subscript{V}{i} \in \N_0$ (or any subset of $\N_0$).
I denote by $\SubscriptNV{V}{i}$ the subscript element $\Subscript{V}{i}$.
A \emph{matrix} $X$ is a rectangular array of numbers $\SubscriptM{X}{i}{j} \in \N_0$ (or any subset of $\N_0$) arranged \st
$$X=
\begin{pmatrix}
  \SubscriptM{X}{\Ibase}{\Ibase} & \SubscriptM{X}{\Ibase}{1} & \cdots & \SubscriptM{X}{\Ibase}{n-1} \\
  \SubscriptM{X}{1}{\Ibase} & \SubscriptM{X}{1}{1} & \cdots & \SubscriptM{X}{1}{n-1} \\
  \vdots & \vdots  & \ddots & \vdots  \\
  \SubscriptM{X}{m-1}{\Ibase} & \SubscriptM{X}{m-1}{1} & \cdots & \SubscriptM{X}{m-1}{n-1}
\end{pmatrix}$$
...
\begin{definition}
The \emph{trie} $\RtrieOf{\Strings}$ of $\Strings$ is a lexicographically ordered tree data structure having one node designated as the root and $\Scard$ leaves, denoted as $\StrieLeaf{\Ibase}, \dots, \StrieLeaf{\Scard}$, where leaf $\StrieLeaf{i}$ points to string $\String^i$.
...
\If {\NotB \Call{isRoot}{$\StrieNode$}}
	\While {$\StrieChar \gets \text{next}_{lex}{\Call{label}{\StrieNode}}$}
