...
I adopt the notation $s_{i \dots j}$ to denote the subscript symbols $s_i \dots s_j$.
I denote the length of $s$ by $|s|=n$.
\end{definition}

\begin{definition}
\label{def:qgram}
I call \emph{$q$-gram} a string of length $q$.
\end{definition}
...
I adopt the notation $V[i]$ to denote the subscript element $v_i$.
\end{definition}

\begin{definition}
\label{def:matrix}
A \emph{matrix} $X=x_{1,1} x_{1,2} \dots x_{1,m}$ is a finite two-dimensional sequence of numbers $x_i \in \N_0$ (or any subset of $\N_0$).
I adopt the notation $X[i,j]$ to denote the subscript element $x_{i,j}$.
\end{definition}

\section{Transcripts, alignments and distances}

I now define basic edit operations to transform one string into another.
...
If the two strings have different lengths, some symbols from $x$ must be necessarily inserted or deleted in order to obtain $y$.
...
Thus each path from the root to a leaf $(i,j)$ spells the suffix $\Sc_{(i,j) \dots}$ (definition~\ref{def:colt}).
\end{definition}
