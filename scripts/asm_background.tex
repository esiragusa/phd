...
In this chapter, we introduce some classic full-text indices (suffix arrays and $q$-gram indices) and subsequently succint full-text indices (various FM-indices) replacing suffix trees.
...
Alternative, we can reduce the average case runtime to $\Oh(m + \log{n})$ without storing any additional information, by using the MLR heuristic \citep{Manber1990}.
...
If we prune our idealized suffix tree to a small depth $q$, we can improve again the query time over the suffix array.
...
\end{eqnarray}
where $p \in \Sigma^q$ is a $q$-gram and the function $\rho : \Sigma \rightarrow [0 \dots \sigma - 1]$ denotes the lexicographic rank of any symbol in the alphabet $\Sigma$.
...
Algorithm~\ref{alg:qgram-exact} runs in $\Oh(q)$ time, but the time to compute the function $h$ can be neglected and  the procedure requires fetching only two memory locations from $D$.
...
If the patterns are shorter or equal to the fixed $q$, we access the suffix array only to locate the occurrences, as the directory $D$ alone is sufficient to count.
...
\end{inparaenum}
The latter property can allow up to compress the suffix array bucket-wise \eg using Elias $\delta$-coding \citep{?} or to implement cache-oblivious strategies to process the occurrences \citep{?}.
