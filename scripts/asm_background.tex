...
A \emph{string} (or word) $\String=\Subscript{\String}{0} \Subscript{\String}{1} \dots \Subscript{\String}{\Slen-1}$ over an alphabet $\Sigma$ is a finite sequence of symbols $\Subscript{\String}{i} \in \Sigma$.
...
\end{definition}
The \emph{lexicographical order} $<_{lex}$ between two non-empty strings $\XSeq,\YSeq$ is defined as $\XSeq <_{lex} \YSeq \iff \Subscript{\XSeq}{0} < \Subscript{\YSeq}{0}, \text{ or } \Subscript{\XSeq}{0} = \Subscript{\YSeq}{0} \text{ and } \Suffix{\XSeq}{1} <_{lex} \Suffix{\YSeq}{1}$.
...
An \emph{array} (or table, or vector) $V=\Subscript{V}{0} \Subscript{V}{1} \dots \Subscript{V}{\Slen-1}$ is a finite sequence of numbers $\Subscript{V}{i} \in \N_0$ (or any subset of $\N_0$).
...
$$X=
\begin{pmatrix}
  \SubscriptM{X}{0}{0} & \SubscriptM{X}{0}{1} & \cdots & \SubscriptM{X}{0}{n-1} \\
  \SubscriptM{X}{1}{0} & \SubscriptM{X}{1}{1} & \cdots & \SubscriptM{X}{1}{n-1} \\
  \vdots & \vdots  & \ddots & \vdots  \\
  \SubscriptM{X}{m-1}{0} & \SubscriptM{X}{m-1}{1} & \cdots & \SubscriptM{X}{m-1}{n-1}
\end{pmatrix}$$
...
\begin{definition}
The \emph{trie} $\RtrieOf{\Strings}$ of $\Strings$ is a lexicographically ordered tree data structure having one node designated as the root and $\Scard$ leaves, denoted as $\StrieLeaf{1}, \dots, \StrieLeaf{\Scard}$, where leaf $\StrieLeaf{i}$ points to string $\String^i$.
