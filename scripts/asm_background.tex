...
Nonetheless, the definition of distance functions between strings lends to the generalization of exact string matching to a more challenging problem: \emph{approximate string matching}.
...
In this manuscript, I often consider \emph{multiple} string matching problems, \ie variants in which many patterns are given at once, instead of \emph{single} problems where they are given one by one in an online fashion.
...
For instance, multiple online methods are allowed to preprocess all patterns and then scan the text only once, while single online methods have to scan the text every time a pattern is provided.
...
\emph{Specificity} and \emph{sensitivity} measure the accuracy of filtering methods.
...
Most practical applications, \eg read mapping, do not require strictly lossless filtration.
...
Hence, efficient filtering methods balance their runtime between filtration and verification.
