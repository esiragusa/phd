...
How to choose the partitioning?
Which length of $q$ makes filtration lossless?
$q=\left \lfloor \frac{m}{k+1} \right \rfloor$.

How many verifications we expect to have?
Let assume the text to be generated according to the uniform Bernoulli model.
The emission probability of any symbol $c$ is
\begin{eqnarray}
p_c = \frac{1}{\sigma} \text{ for all } c \in \Sigma
\end{eqnarray}
and under \iid the probability of any $q$-gram is simply
\begin{eqnarray}
\text{Pr}(H > 0) = \frac{1}{\sigma^q}
\end{eqnarray}
and the expected number of occurrences of a $q$-gram in a text of length $n$ is
\begin{eqnarray}
E(H) = \sum_{i=1}^{n-q+1}{\text{Pr}(H > 0)} = \frac{n - q + 1}{\sigma^q} \leq \frac{n}{\sigma^q}
\end{eqnarray}
thus the expected number of verifications produced by filtration with exact seeds is
\begin{eqnarray}
E(V) = (k + 1) \cdot E(H) < \frac{n (k + 1)}{\sigma^q}
\end{eqnarray}

Which is the runtime of the algorithm?
If we use the classic DP algorithm of section~\ref{sec:introonline} to verify candidate locations, we obtain a $k$-differences algorithm that is sublinear for an error rate
\begin{eqnarray}
\epsilon < \frac{m}{\log_{\sigma}{m}} < \frac{1}{\log_{\sigma}{m}}
\end{eqnarray}
