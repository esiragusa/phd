...
d_E(x_{1 \dots i},\epsilon)&=&i \; \text{ for all } 1 \leq i \leq m\\
d_E(\epsilon, y_{1 \dots j})&=&j \; \text{ for all } 1 \leq j \leq n
\end{eqnarray}
and the recursive case for all $1 < i \leq m$ and $1 < j \leq n$ is as follows:
\begin{eqnarray}
...
\State {$D[0,0] \gets 0$}
\For {$i \gets 1 \, \To m$}
	\State {$D[i,0] \gets D[i-1,0] + 1$}
\EndFor
\For {$j \gets 1 \, \To n$}
	\State {$D[0,j] \gets D[0,j-1] + 1$}
	\For {$i \gets 1 \, \To m$}
		\State {$D[i,j] \gets \min \, \{ \, D[i-1,j] + 1, D[i,j-1] + 1, D[i-1,j-1] + \delta(t_j, p_i) \, \}$}
	\EndFor
\EndFor
\State \Return D[m,n]
\end{algorithmic}
...
Algorithm \ref{alg:dp-edit} computes the above recurrence relation in time $\Oh(nm)$ using a dynamic programming table $D$ of $(m+1) \times (n+1)$ cells, where cell $D[i,j]$ stores the value of $d_E(x_{1 \dots i},y_{1 \dots j})$.
