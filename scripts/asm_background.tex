...
\begin{figure}[h]
\begin{center}
\caption{Example of edit transcript transforming the string $x=AAAA$ into $y=CCCC$. The transcript character M indicates a match, R a replacement, I an insertion, and D a deletion.}
...
We define a dotplot trace to be a path in the matrix connecting non-decreasing positions of the matrix.
...
\begin{figure}[h]
\begin{center}
\caption{Dotplot of the strings $x=AAAA$ and $y=CCCC$.}
\label{fig:dotplot}
...
\end{itemize}
and by defining the total cost $C(z)$ of an alignment between two strings as the sum of the weights of all its alignment symbols:

\begin{eqnarray}
C(z) = \sum_{i=0}^{|z|}{\omega(z_i)}
\end{eqnarray}

In particular, the \emph{Hamming distance} between two strings $x,y \in \Sigma^{n}$ is defined as the function $d_H : \Sigma^{n} \times \Sigma^{n} \rightarrow \N$ counting the number of substitutions necessary to transform $x$ into $y$.
We obtain the Hamming distance by defining for all $(\alpha,\beta) \in \Sigma \times \Sigma$, $\omega(\alpha,\beta) = 1$ iff $\alpha \neq \beta$ and 0 otherwise, and by setting all $\omega(\alpha,-)$ and $\omega(-,\beta)$ to be $\infty$ in order to disallow indels.

The edit or \emph{Levenshtein distance} between two strings $x,y \in \Sigma^{*}$ is defined as the function $d_E : \Sigma^{*} \times \Sigma^{*} \rightarrow \N$ counting the \emph{minimum} number of edit operation necessary to transform $x$ into $y$.
It is obtained by defining $\omega(\alpha,\beta)$ as in the Hamming distance, $\omega(\alpha,-)$ and $\omega(-,\beta)$ as 1, and by taking the minimum cost over all possible alignments of $x,y$:
\begin{eqnarray}
d_E(x,y) = \sum_{z \in \mathbb{A}(x,y)}{C(z)}
\end{eqnarray}
...
The problem of finding an optimal alignment between two strings is the dual of the edit distance problem.
...
Given two strings $x,y$, for all $1 \leq i \leq n$ and $1 \leq j \leq m$ we define with $d_E(x_{1..i},y_{1..j})$ the edit distance between their prefixes $x_{1..i}$ and $y_{1..j}$.
The base conditions of the recurrence relation are:
\begin{eqnarray}
d_E(\epsilon,\epsilon)&=&0\\
d_E(x_{1..i},\epsilon)&=&\sum_{l=1}^{i}{\omega(x_l, -)} \text{ for all } 1 \leq i \leq n\\
d_E(\epsilon, y_{1..j})&=&\sum_{l=1}^{j}{\omega(-, y_l)} \text{ for all } 1 \leq j \leq m\\
\end{eqnarray}
and the recursive case is defined as follows:
\begin{eqnarray}
d_E(x_{1..i},y_{1..j}) = \min \left\{
\begin{array}{lcl}
d_E(x_{1..i-1},y_{1..j})&+&\omega(x_i, -)\\
d_E(x_{1..i},y_{1..j-1})&+&\omega(-, y_j)\\
d_E(x_{1..i-1},x_{1..j-1})&+&\omega(x_i, y_j)
\end{array}
\right.
\end{eqnarray}
