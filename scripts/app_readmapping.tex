...
We evaluate Yara through three experiments: \begin{inparaenum}[(i)]
...
Below, \texttt{MIN} and \texttt{MAX} are placeholders for minimal and maximal insert size; alternatively, \texttt{INS} is the mean insert size and \texttt{ERR} its allowed deviation, \ie \texttt{INS = (MIN + MAX) / 2}, \texttt{ERR = (MAX - MIN) / 2}.

\paragraph{Yara}
Version ??? was used.
In order to use Yara as an all-mapper, we passed the argument \texttt{--all}, otherwise the tool runs as a best-mapper by default.
...
%Here we compare the accuracy of Yara with other tools on simulated data.

This experiment measures the ability of each tool to report the \emph{original} location of any simulated read.
Each tool can output at most one mapping location per read.
This experiment is suited at evaluating best-mappers.
This experiment is independent of the scoring scheme used by any tool, \eg edit distance or quality based.

It does not consider mapping qualities. Applying a mapping quality cutoff lowers recall and increases precision.

Tools have to map all reads twice, first in single-end mode and then using additional paired-end information.

The categories precision and recall require a 
A read is mapped \emph{correctly} if the mapper reports its original location, 
and it is mapped \emph{uniquely} if the mapper reports only one location.
Rabema defines \emph{recall} to be the fraction of reads which were correctly mapped and \emph{precision} the fraction of uniquely mapped reads that were mapped correctly.
...
The Rabema benchmark~\citep{Holtgrewe2011} (v1.1) considers reads with multiple mapping locations.
%The previous experiment doesnt.
%This benchmark provides sensitivity evaluation.
%It is only single-end.

The benchmark consists of the categories \emph{all}, \emph{all-best}, \emph{any-best}.
A read mapper has to find all, all of the best, or any of the best edit distance locations for each read.
Mapping locations in each category are classified by their error rate.
...
We performed the benchmark for an error rate of 5\,\%.
Therefore, we built a Rabema gold standard for each dataset by running RazerS\,3 in full-sensitive mode up to 5\,\% error rate.
