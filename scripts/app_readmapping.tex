...
Phred base quality scores have been introduced in \citep{Ewing1998, Ewing1998b} to assess the quality of sequencing single bases in capillary reads.
...
Genomes evolved through multiple types of duplication events, including whole-genome duplications \citep{?} or large-scale segmental duplications in chromosomes \citep{?}, transposition of repetitive elements as short tandem repeats (microsatellites) and interspersed nuclear elements (LINE, SINE) \citep{?}, proliferation of repetitive structural elements such as telomeres and centromeres \citep{?}.
...
\end{inparaenum}
It goes without saying that read mapper and downstream analysis tools must agree on a common strategy.
...
These paradigms are valid not only for read mappers but also for any downstream analysis tool, \eg variant callers.
...
Compared to best-mapping, all-mapping algorithms can potentially detect a wider spectrum of genomic variation events.
For instance, CNVs are detectable under the all-mapping paradigm \citep{mrCaNaVaR}.
An all-mapping variant caller detects SNVs in homologous regions \citep{Sniper}.
 % transpositions~\citep{VariationHunter} ?

A practical challenge of all-mapping is represented by reporting and handling the resulting datasets of mapping locations.
 which can have a size up to two orders of magnitude bigger compared to the corresponding input read sets.
Sometimes all is too much.

% -----------------------------------------------------------------------------

\section{Limits of read mapping}
\label{sec:mappability}

%Reassembling a genome from HTS reads is a non-trivial task, not only in \emph{de novo} assembly but also in \emph{reference-guided} plans.
%The difficulty of this task is caused by two aspects of the sequencing data: the shortness of the reads and the artefacts they contain.
%Sequencing artefacts can be spotted by looking at consensus alignments obtained from piled-up reads, and eventually at their base quality scores.
%It goes without saying that this method of error correction is prerogative of confident consensus alignments.

%As the confidence of consensus alignments increases with higher coverage, in general, deeper sequencing helps to recover from sequencing errors.
%Still, alignment confidence is strictly related to read length, in conjunction with a third aspect strictly inherent to the donor genome: its repeats content.
%Some reads cannot be confidently assigned to one single mapping locations.
...
\subsection{Genome alignability}

We now give a definition of genome alignability analogous to \citep{Derrien2012}.
