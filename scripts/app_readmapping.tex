...
\citeauthor{Derrien2012} quantify the \emph{unique mappability} of whole genomes for human, mouse, fly, and worm.
Concerning human, at $(36,2)$-mapping, about 30~\% of the whole genome is not uniquely mappable.
...
To estimate resequencing accuracy at a single locus, \citeauthor{Derrien2012} consider the mappability of all possible $q$-grams spanning it.
...
Genome mappability score (GMS) \citep{Lee2012}, analogously to pileup mappability, estimates single-locus resequencing accuracy.
...
Subsequently, they derive average mapping probability of any read spanning a location $l$ of a reference genome $g$ as\footnote{Equation~3 in \citep{Lee2012} is not precise, please consider to equation~\ref{eq:gms}.}:
\begin{eqnarray}
\label{eq:gms}
...
\end{eqnarray}
Thus, fixed a genomic sequence $g$, they define the genome mappability score $\text{GMS}(l)$ as its percentual value:
\begin{eqnarray}
\text{GMS}(l) = 100 \, Q(l|g)
\end{eqnarray}

\citeauthor{Lee2012} use GMS to estimate which fractions a genome are hard to resequence.
%First, they simulate reads having length and error profiles similar to those issue by actual sequencing technologies.
%Then, they map them with BWA (see section \ref{sec:background:mappers:bwa}).
They define \emph{low GMS} regions as those locations for which $\text{GMS}(l) \leq 10$, and measure the percentage of such locations in the human genome.
