...
\section{Surveys on read mappers}
...
Bowtie is a mapper designed to have a small memory footprint and quickly report a few good mapping locations for early generation Illumina/Solexa and AB/SOLiD short reads of length up to 50~bp.
...
Each mapping location is verified using a striped vectorial dynamic programming algorithm, implemented using SIMD instructions, previously introduced by \cite{Farrar2009} and extended to compute end-to-end alignments.
...
BWA-backtrack is designed to map Illumina/Solexa reads up to 100~bp and report a few best end-to-end alignments.
...
BWA-SW is designed to map Roche/454 reads of average length of 400~bp.
...
Soap~2 is very similar to Bowtie: it has been designed to produce a very quick but shallow mapping of Illumina/Solexa reads up to 75~bp with no more than 2 mismatches and no indels.
...
RazerS has been designed to report all mapping locations within a fixed hamming or edit distance error-rate.
...
RazerS~3 is a faster version featuring shared-memory parallelism, a faster banded-Myers verification algorithm, and a faster filtration scheme based on exact seeds that however turns out to be very weak on mammal genomes.
...
The tools mrFast and mrsFast are designed to report all mapping locations within a fixed absolute number errors, respectively under the hamming and edit distance, given Illumina/Solexa reads of length ranging from 50~bp to 125~bp.
...
The GEM mapper is a flexible read aligner that can be used either as an all-mapper or as a best/unique-mapper.
It is based on state of the art approximate string matching methods like approximate seeds and suffix filters.
GEM indexes the reference genome with an FM-index, tries to find an optimal filtration strategy per read, and verifies candidate locations using Myers algorithm.
...
Pros: full-sensitive, search by strata, all/best/unique strategies.
Cons: many parameters, indirect SAM output, bad paired-end, closed source.
