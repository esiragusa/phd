...
\section{Paradigms of read mapping}
\label{sec:mappability}

The goal of secondary analysis is to build a consensus alignment 
This requires confidently mapping reads.



The difficulty of confidently mapping a significant fraction of HTS reads comes from the non-random nature of genomes.
...
%In the case of HTS read mapping tools, the first evident effect of the heavy tail in the $k$-mer distribution of reference genomes is the dramatic loss of specificity in certain regions, which increases the computational cost of programs based on filtration methods.
...
According to \citeauthor{Treangen2011}, common strategies to deal with \emph{multi-reads} are
\begin{inparaenum}[(i)]
...
% We divide them in all and best-mapping.
