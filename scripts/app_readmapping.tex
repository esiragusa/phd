...
In the engineering section, we explain how our filtration method works, which data structures we adopt for indexing, and how we perform seed extension.
...
Our strongest requirement is to build on a non-heuristic and full-sensitive filtration.

RazerS, the read mapper developed by \citeauthor{RazerS}, was based on a $q$-gram filter.
Gapped $q$-grams are not easy to generalize to edit distance. Contiguous $q$-grams are much slower than exact seeds for moderate error rates. 
RazerS~3 went back to exact seeds, similarly to mr(s)Fast. Which is fine for short reference genomes but becomes too weak for mammal genomes; clearly, a stronger filtration strategy would had been beneficial.

Filtration specificity in terms of candidate locations to be verified is strongly correlated to seed length.
...
We rediscovered the works of \citeauthor{Navarro2000} and \citeauthor{Myers1994}. Approximate seeds are a generalization of exact seeds providing not only stronger but flexible filtration. This generalization is full-sensitive and works for edit distance.
%In particular, our filtration method is based on exact or approximate seeds; by employing approximate seeds instead of exact seeds, we obtain a stronger filter for long reference genomes, which is still non-heuristic and quasi full-sensitive.
