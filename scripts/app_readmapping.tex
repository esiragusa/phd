...
To improve the efficiency of our application, we implemented a generic top-down traversal for some additional indices, namely the suffix array (SA) \citep{Manber1990}, the $q$-gram index, and various flavours of the full-text minute index (FM-index) \citep{Ferragina2001}.
...
We initially chose the ESA over LST because of construction times, as both our ESA and LST implementations require $13$ bytes per base pair and comparable query speed.
...
Thus, for the human reference genome (GRCh38), we had a suffix trie constructed in about 90~minutes and consuming 39~GB of memory (see figure~\ref{fig:index:construction}).
...
indeed, the lookup table turned out to be useful when searching patterns one by one, but not when coupled with our multiple backtracking algorithm, performing a factorization of the top-down traversal.
...
Our initial FM-index consumed about $1.5$ bytes per base pair with a SA sampling of 10~\%, thus the memory footprint of the index went down to 4.5~GB, but Masai became significantly slower (less than twice as slow though).
...
we index only the first suffix of any string in the collection and construct the SA-based trie in time $\Oh(n \log n)$ via quicksort; the top-down traversal based on binary search still works.
We can reuse also the LST: it is sufficient to analogously fill the partial SA in the collection and then apply the \emph{wotd}-algorithm \citep{Giegerich1999} to construct the trie in linear time.
Quicksorting the SA turned out to be faster than radix sorting the LST but, in our application, the LST payed off in query time.
Indeed, the LST explicitly contains all trie nodes, as opposed to the SA.
