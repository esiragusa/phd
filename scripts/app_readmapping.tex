...
Two surveys \citep{Li2010, Fonseca2012} try to help bioinformaticians to extricate themselves from the jungle of read mapping.
...
The survey primarily considers the application of SNP calling; in the considered setup, mappers enumerating a comprehensive set of locations always lag behind mappers designed to report only one location per read.
...
\emph{involves application-specific requirements such as how well it work in conjunction with downstream analysis tools (\ie variant callers)}.
...
Variant callers in this category implement calling algorithms able to detect a wider spectrum of genomic variation, like CNVs~\citep{mrCaNaVaR}, long indels~\citep{SPLITREAD}, transpositions~\citep{VariationHunter}, as well as SNVs in low-mappability regions~\citep{Sniper}.
...
%The vast majority of NGS analysis pipelines consider one single mapping location \citep{Survey on pipelines?}, simply because it is straightforward to think in these terms.
%Such pipelines expect a confidence level, \ie a mapping quality, along with one mapping location per read.
%Reads not mapping confidently to one single location are discarded.
%Despite being now de-facto standard, this method prevents the analysis of low mappability regions \citep{Lee2011}.
%Despite these limitations, the so called best-mapping is the most asked type of read mapping.

%Some pipelines accept or require multiple mapping locations per read \citep{Survey on all-mapping pipelines?}.
%This workflow improves small variant calling \citep{Sniper?}, and is required for the detection of structural variants \eg CNVs.
%This use case requires an exhaustive mapping, called all-mapping.
...
%We stratify mapping locations by error rate.

%How?
%One na\"ive solution consists into applying filters from 0\,\% error rate to the 
%Greedy strategy: sort the seeds by number of hits.

%Yara randomly chooses the primary mapping location of a read among its co-optimal mapping location.
