...
The reader is thus referred to the next chapter (section~\ref{sec:yara:engineering}) for the complete description of an efficient best-mapping method that, in standard scenarios, is an order of magnitude faster than all-mapping.
...
I present a generalization of the pigeonhole lemma to strata...

I propose a greedy strategy: sort the seeds by number of verifications to perform.
...
Indeed, navigation towards a child node involves the computation a subinterval of the current suffix array interval; such computation accesses memory locations within the current interval, that have good chances to be in the cache.
...
Multiple backtracking factorizes navigation of top nodes, thus it pays off with forward search rather than with backward search.
